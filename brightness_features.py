...
Brightness features: ROI pixel intensity, log ratios and temporal derivatives.
"""
...
class PixelBrightnessExtractorOptimized:
    ...
    def _extract_vectorized(self, video_file, label, pix_threshold,
        ...
        # Back-fill the leading rows (a frame mask / stride that starts at
        # frame k leaves 0..k-1 empty). Dropping them instead would return a
        # table shorter than num_frames with row i no longer being frame i.
        first = next((b for b in brightness_features if b is not None), None)
        if first is None:
            # nothing decoded at all: keep the frame count, values NaN
            return pd.DataFrame(index=range(num_frames))
        for i in range(num_frames):
            if brightness_features[i] is None:
                brightness_features[i] = first
            else:
                break
