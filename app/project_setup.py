...
class ProjectSetupWizard(tk.Toplevel):
    ...
    def _show_step1(self):
        ...
        if recent:
            ...
            self._add_example_button(btn_frame, row=1, columnspan=2)
        else:
            ...
            self._add_example_button(btn_frame, row=2, columnspan=1)
    ...
    def example_project_dir():
        """The example project shipped next to the app (release zip and
        install folder), or None when it is not there."""
        d = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'example_project')
        return d if os.path.isfile(os.path.join(d, 'PixelPaws_project.json')) else None

    def _add_example_button(self, parent, row, columnspan):
        """One-click open of the shipped example project (six clips with a
        key file, nothing precomputed), so a fresh install has something to
        run the pipeline on."""
        d = self.example_project_dir()
        if not d:
            return
        ttk.Button(parent, text="🐾  Open the example project",
                   width=46 if columnspan == 2 else 30,
                   command=lambda: self._open_folder(d)).grid(
                       row=row, column=0, columnspan=columnspan, padx=8, pady=(10, 4))
        ttk.Label(parent, text="Six 60 s formalin clips with a key file. Run the "
                               "Quick Start pipeline on them to see every step.",
                  foreground='gray', wraplength=420, justify='center').grid(
                      row=row + 1, column=0, columnspan=columnspan)
    ...
    def _new_project(self):
        ...
        moved = gather_loose_media(self.project_folder)
    ...
    def _note_moved(self, moved):
        """Tell the user which loose files just went into videos/ (top of step 2)."""
        if not moved:
            return
        n_h5 = sum(1 for f in moved if f.lower().endswith('.h5'))
        n_vid = len(moved) - n_h5
        parts = ([f"{n_vid} video{'s' if n_vid != 1 else ''}"] if n_vid else []) + \
                ([f"{n_h5} .h5 file{'s' if n_h5 != 1 else ''}"] if n_h5 else [])
        lbl = ttk.Label(self.content, text=f"Moved {' and '.join(parts)} from the project folder into videos/",
                        foreground='green', font=(FONT_FAMILY, 9))
        kids = self.content.winfo_children()
        lbl.pack(side='top', anchor='w', pady=(0, 4), before=kids[0] if kids else None)
    ...
    def _show_step2(self):
        ...
        # score-first path is Quick Start / Run Classifiers with those. This field
        ...
        hint = ttk.Label(
            ...
            text=("Optional - only needed if you plan to train your own classifiers. "
                  ...
                  "this and use the Quick Start or Run Classifiers tab after setup."))
        ...
        ttk.Label(cf, text="Square ROI Size (px):").grid(
            row=row, column=0, sticky='w', pady=4)
        # Seed from square_size when present: that is the key the feature
        # cache / main GUI actually use, roi_size is only the wizard's copy.
        _sq = existing.get('square_size') or []
        _roi_seed = existing.get('roi_size', _sq[0] if _sq else 20)
        self._roi_size = tk.StringVar(value=str(_roi_seed))
    ...
    def _save_step2_config(self):
        ...
        if _PROJECT_CONFIG_AVAILABLE:
            ...
            cfg.roi_size = roi
            # square_size is what the feature-cache hash and the main GUI's
            # Feature Configuration actually read; Step 3 extracts with
            # square_size=[roi]. Leaving the dataclass default ([40]) here
            # meant wizard-built caches never matched and were recomputed.
            cfg.square_size = [roi]
    ...
    def _open_key_file_dialog(self):
        ...
        if os.path.isfile(key_path):
            try:
                import csv
                # utf-8-sig: Excel's "CSV UTF-8" BOM would otherwise make the
                # header '﻿Subject' and every row load as Subject ''.
                n_blank = 0
                dupes = []
                with open(key_path, newline='', encoding='utf-8-sig') as f:
                    for row in csv.DictReader(f):
                        ...
                        if subj in existing:
                            dupes.append(subj)
                        existing[subj] = (row.get('Treatment') or '').strip()
                if not existing:
                    print(f"Warning: key file {key_path} has no Subject rows "
                          f"(needs 'Subject' and 'Treatment' columns)")
                elif n_blank:
                    print(f"Warning: key file: {n_blank} row(s) with an empty "
                          f"Subject were ignored")
                if dupes:
                    print(f"Warning: key file: duplicate Subject row(s) "
