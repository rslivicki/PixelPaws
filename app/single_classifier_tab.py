...
# Metric dropdown: (column, aggregation across bins, label).  Per-subject
# values go through analysis_core.per_subject_metric, which overrides the
# 'mean' rows: mean bout duration is bout-weighted, % time and bout
# frequency are whole-session rates over the covered time.
_METRICS = [
...
class SingleClassifierTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        _T(ttk.Button(df_, text="🏷 Review scoring…",
                      command=lambda: self.app.open_scoring_preview()),
           "Play a scored session with every classifier's calls drawn on "
           "the video.").pack(anchor="w", padx=6, pady=(2, 0))
        ...
        # DoubleVar: 29.97 fps must stay 29.97 (the core bins by float fps;
        # rounding to 30 makes every bin drift).
    ...
    def on_project_changed(self):
        ...
        self._scan_status.set("No project scanned.")
        self._fps_from_project()
        self.scan_project_folder()

    def _fps_from_project(self):
        """Start the FPS box at the project's video frame rate, so a 30 fps
        project is not silently binned as 60 fps (the box's default).
        Left alone when no project video can be read."""
        import glob
        vd = os.path.join(self._project() or "", "videos")
        vids = sorted(p for e in ("mp4", "avi", "mov", "mkv")
                      for p in glob.glob(os.path.join(glob.escape(vd), f"*.{e}"))
        ...
        for p in vids[:3]:
            try:
                ...
                continue
            if fps and fps > 0:
                self.fps_var.set(round(float(fps), 2))
                return
    ...
    def _load_key(self, path):
        ...
        try:
            # relpath raises on Windows when the key sits on another drive
            shown = (os.path.relpath(path, self._project())
                     if self._project() else path)
        except ValueError:
            shown = path
        self.key_file_var.set(shown)
    ...
    def _auto_fps(self):
        ...
        try:
            ...
            if fps > 0:
                self.fps_var.set(round(float(fps), 2))
                self._status.set(f"FPS detected: {fps:.2f}")
        except Exception as e:
    ...
    def run_analysis(self, silent=False):
        ...
        if self.key_df is None and files:
            # No key file: one mouse, or a pilot with no groups yet. Run everything
            # as one group rather than refuse; loading a key later replaces this.
            self.key_df = core.one_group_key(
                files, self.filename_prefix_var.get().strip())
            self._key_status.set("no key file: all sessions in one group")
        if not files:
            if not silent:
                if self._files:
        ...
        # The core sends one message per file (plus one for the combined
        # pass); the bar steps once per message.
        n_steps = len(files) + (1 if cfg.analyze_mode in ("combined", "both")
                                else 0)
        self._progress.configure(value=0, maximum=max(n_steps, 1))
        ...
        def _worker():
            try:
                res = core.run_analysis(
                    ...
                    progress_cb=lambda m: self._run_queue.put(("prog", m)),
                    cancel=self._cancel)
    ...
    def _poll_run(self):
        try:
            while True:
                ...
                if msg[0] == "prog":
                    _, m = msg
                    cur = float(self._progress["value"])
                    self._progress.configure(
                        value=min(cur + 1, float(self._progress["maximum"])))
                    self._status.set(m)
                elif msg[0] == "done":
    ...
    def _df(self, include_phase=False):
        ...
        if w > 0 and "Bin_Start_Min" in df.columns:
            keep = df["Bin_Start_Min"] < w
            if "Bin_Index" in df.columns:
                # phase rows (Bin_Index < 0) carry no Bin_Start_Min; the
                # window applies to bins only, never to them
                keep |= df["Bin_Index"] < 0
            df = df[keep]
        return df.copy()
    ...
    def _per_subject(self, df, col, agg):
        """{group: per-subject values}. Mean bout duration is bout-weighted
        in the core (NaN = no bouts; those subjects are left out)."""
        import analysis_core as core
        per = core.per_subject_metric(df, col, agg)
        per = per.dropna(subset=[col])
    ...
    def _stats_text(self):
        ...
        how = {"Mean_Bout_Duration_s": "bout-weighted mean across bins",
               "Percent_Time": "time in behavior / time covered by the bins",
               "Bout_Frequency_per_min": "bouts / minutes covered by the bins",
               }.get(col, f"{agg} across bins")
        lines.append(plot_style.stats_table(
            f"Per-subject {lab} ({how})", [lab], rows_by_g,
            opts, test_fn=None, unit=lab))
        try:
            res = core.perform_statistical_test(
                {g: per[g] for g in groups}, alpha=alpha, paradigm=paradigm)
        except ValueError as e:
            # e.g. scipy "All numbers are identical" - keep the rest
            res = {"test_type": "omnibus", "p_value": float("nan"),
                   "note": f"no variance ({e})"}
        lines.append("")
        if res and res.get("note"):
            tail = (f"; {lab} is identical in every subject, nothing to test."
                    if "all values identical" in res["note"] else ".")
        ...
        if self._view.get() in ("timecourse", "traces",
            ...
            try:
                ...
                if an:
                    lines.append(f"Two-way ANOVA ({lab}; between-subjects, "
                                 f"bins treated as independent):")
                    for k, v in an.items():
                        lines.append(f"  {k}: {v}")
            except Exception as e:
    ...
    def _view_dataframe(self):
        ...
        if view == "bouts":
            import analysis_core as core
            per = core.per_subject_metric(binned, "N_Bouts", "sum").merge(
                core.per_subject_metric(binned, "Mean_Bout_Duration_s",
                                        "mean"),
                on=["Subject", "Treatment"], how="left")
        ...
        import analysis_core as core
        per = core.per_subject_metric(binned, col, agg)
    ...
    def _write_sidecar(self, csv_path):
        ...
        cfg = self._build_cfg()
        try:
            from pp_version import __version__ as pp_ver
        except Exception:
            pp_ver = None
        meta = {
            "pixelpaws_module": "single_classifier_tab.py",
            "pixelpaws_version": pp_ver,
            "git_sha": sha,
