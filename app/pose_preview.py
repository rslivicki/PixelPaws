...
def keep_colors(widget_cls, master, **kw):
    """Build a classic tk widget whose explicit colors survive the app theme.

    ttkbootstrap repaints classic tk widgets with the theme's fg/bg: at
    construction under its tkinter patch (1.x), and on every theme switch
    (2.x walks the whole widget tree). autostyle=False opts out, but plain
    tkinter and unpatched 2.x reject that option, so retry without it;
    _tb_no_autostyle is the flag the 2.x theme walk honors.
    """
    try:
        w = widget_cls(master, autostyle=False, **kw)
    except tk.TclError:
        w = widget_cls(master, **kw)
    w._tb_no_autostyle = True
    return w


def h5_for_video(video_path):
    """The pose .h5 next to a video (the active model's first), or None.
    Globs {stem}DLC* so S1 never picks up S10's pose file."""
    from h5_resolve import rank_h5
    stem = os.path.splitext(os.path.basename(video_path))[0]
    pat = os.path.join(glob.escape(os.path.dirname(video_path)),
                       glob.escape(stem) + "DLC*.h5")
    h5s = rank_h5(glob.glob(pat))
...
def draw_keypoints(img, pose, frame_idx, thr=0.6, labels=False, trail=False):
    """Draw every body part above the likelihood gate on a BGR frame, in
    place: a colored dot with a white ring, optional name and 30-frame
    trail. Shared by the pose preview and the scoring review."""
    import cv2
    for i, (bp, (x, y, p)) in enumerate(pose.items()):
...
def open_pose_preview(root, project_folder, video_path=None):
    ...
    def _draw(frame_idx, img):
        return draw_keypoints(img, state["pose"], frame_idx,
                              thr=float(thr_var.get()),
                              labels=lbl_var.get(), trail=trail_var.get())
