...
class ProjectConfig:
    ...
    def load(cls, project_folder: str) -> 'ProjectConfig':
        ...
        # The folder we were asked to load from wins over the path stored in
        # the JSON: projects get moved (Box, OneDrive, the example project
        # unzipped anywhere) and a stale absolute path would point the app at
        # a folder that no longer exists.
        cfg.project_folder = project_folder

        # Legacy: roi_size → square_size. square_size is the key the feature
        # cache hash and the main GUI read; roi_size is the setup wizard's
        # single-value view of it. Keep the two consistent in both directions
        # so a wizard-built project and the GUI hash the same cache key.
        if 'square_size' not in data and 'roi_size' in data:
            rs = data['roi_size']
            cfg.square_size = [int(rs)] if not isinstance(rs, list) else [int(x) for x in rs]
        elif 'roi_size' not in data and data.get('square_size'):
            try:
                cfg.roi_size = int(list(data['square_size'])[0])
...
MEDIA_EXTS = ('.mp4', '.avi', '.mov', '.mkv', '.h5')


def gather_loose_media(project_folder: str):
    """Move videos and DeepLabCut .h5 files sitting in the project root into videos/.

    People often point New Project at a folder that just holds their videos. The app only
    looks in videos/, so those files would be invisible. Top level only; a file whose name
    is already taken in videos/ stays put. Returns the moved file names."""
    import shutil
    if not project_folder or not os.path.isdir(project_folder):
        return []
    videos_dir = os.path.join(project_folder, 'videos')
    loose = sorted(f for f in os.listdir(project_folder)
                   if f.lower().endswith(MEDIA_EXTS)
                   and os.path.isfile(os.path.join(project_folder, f)))
    moved = []
    for f in loose:
        ...
        try:
            ...
            shutil.move(os.path.join(project_folder, f), dst)
            moved.append(f)
        except OSError:
            continue
    return moved
