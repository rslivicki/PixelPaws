...
STAGES = [
    ...
    ("gait",        "Paw contour analysis",                  "OCgait"),
]
...
class OneClickTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        # the two viewers get their own row: on one row with the selection
        # buttons, "Review scoring" was squeezed to "Review scc"
        views = ttk.Frame(sess)
        views.pack(side="bottom", fill="x", pady=(8, 0))
        _pv = ttk.Button(views, text="🎬 Check tracking…",
        ...
        _sv = ttk.Button(views, text="🏷 Review scoring…",
                         command=self._open_scoring_preview)
        _sv.pack(side="left", padx=(6, 0))
        Tip(_sv, "Play a scored video with the classifier calls overlaid -\n"
                 "check the labeling before trusting the graphs.")
        ...
        # Walking-paws activity indicator: paws march while the pipeline is running.
        self._PAW_MAX = 8
        self._paw_n = 0
        ...
        Tip(self._stage_lbls["gait"],
            "Runs the Paw Contour analysis with the manuscript "
        ...
        self._jump_gait = ttk.Button(
            jump, text="Open Paw Contour",
            command=lambda: self._jump("\U0001f43e Paw Contour"),
            state="disabled")
    ...
    def _open_scoring_preview(self):
        ...
        if sel:
            ...
            if v:
                vp = v["path"]
        open_scoring_preview(self.winfo_toplevel(), self._project(), vp)
    ...
    def _start_pose_stage(self):
        ...
        try:
            ...
            # stale active_bundle.txt / bundle folder deleted by hand: fail
            # the stage instead of leaving the tab stuck with Run disabled
            self._fail_stage("pose", f"could not load the DLC model bundle: {e}")
            return
        ...
        # ffprobe tells an H.265 file (copied as is) from one to encode;
        # without it every file would be re-encoded.
        do_transcode = bool(want_transcode
                            and _sh.which("ffmpeg") is not None
                            and _sh.which("ffprobe") is not None)
        if want_transcode and not do_transcode:
            self._log_line("Transcode skipped: ffmpeg and ffprobe were not "
                           "found on PATH (the installer includes them).")
            if self._stage_state.get("transcode") == "pending":
                self._set_stage_silent("transcode", "skipped")
    ...
    def _poll_pose(self):
        ...
        if not alive and self._pose_h5s is None:
            # The tracking thread has ended, but the dialog hands its results over when it
            # reads "all_done" from its message queue, on its own timer. Everything the
            # thread wrote is already in that queue, so read it now. Without this a thread
            # that ended between two of the dialog's reads looked like "nothing tracked".
            try:
                dlg._drain_queue()
            except Exception:
                pass
        try:
            ...
            # The dialog labels a video "(i/n) transcode X.mp4"; the initial
            # placeholder "-" means nothing has started yet.
            _active = cur_txt.strip() not in ("", "-")
            in_transcode = " transcode " in f" {cur_txt} "
            # stage flip transcode -> pose
            if (not in_transcode and _active
        ...
        if self._pose_h5s is None:
            self._pose_h5s = []
        n_asked = len(getattr(self, "_pose_list", None) or [])
        n_new = len(self._pose_h5s)
        if n_new < n_asked:
            n_new = max(n_new, self._tracked_on_disk(self._pose_list))
        if "pose" in self._plan and self._stage_state.get("pose") in ("running", "pending") \
                and n_asked and n_new == 0:
            # The tracking process ran and gave back nothing (it could not start,
            # DeepLabCut is missing, the model failed to load...). Without this the
            # step showed a tick and every later step quietly skipped each video.
            if self._stage_state.get("transcode") == "running":
                self._set_stage_silent("transcode", "done")
            self._fail_stage("pose", f"no video was tracked (0 of {n_asked}). "
        ...
        if n_new:
            self._log_line(f"Pose done: {n_new} video(s) tracked."
                           if n_new >= n_asked else
                           f"Pose done: {n_new} of {n_asked} video(s) tracked. "
                           "See the log for the ones that failed.")
        self.on_project_changed()
        self._advance(None)

    @staticmethod
    def _tracked_on_disk(asked):
        """How many of the asked videos have a pose file from the active model beside
        them. The ground truth for "was anything tracked": the results list arrives by
        callback and must not be the only evidence."""
        ...
        for v in asked or []:
            vp = Path(v["path"])
            h5s = list(vp.parent.glob(f"{vp.stem}DLC*.h5"))
            if any((not scorer) or scorer in h.name for h in h5s):
                n += 1
        return n
    ...
    def _start_batch_stage(self):
        ...
        try:
            ...
            return
        if not ok:
            # Early-out in the app (fps pre-flight declined, bundled
            # classifiers missing, batch already running): nothing was
            # scored, so do not let _poll_batch tick the stage as done.
            self._fail_stage("classifiers",
                             "classifier batch did not start (see the "
                             "Run Classifiers tab).")
            return
        ...
        if failed:
            ...
            self._status_lbl.config(
                text="All done - graphs are ready in the Analyze tabs; "
                     "🏷 Review scoring plays the calls on video."
            ...
            if done:
                self.after(200, self._after_success)

    def _after_success(self):
        """Refresh what the batch refresh does not reach, then make the
        key-file offer Run Classifiers makes (held back during the run so
        it cannot interrupt a step)."""
        if self._stage_state.get("pose") == "done":
            try:
                lt = getattr(self.app, "locomotion_tab", None)
                if lt is not None:
                    lt.scan_sessions()
            except Exception:
                pass
        if self._stage_state.get("classifiers") != "done":
            return
        at = getattr(self.app, "analysis_tab", None)
        if at is None or getattr(at, "key_df", None) is not None:
            return
        if messagebox.askyesno(
                "No key file",
                "Scoring is done, but this project has no key file (which "
                "subject is in which group), so the group graphs are empty."
                "\n\nCreate one now?", parent=self):
            try:
