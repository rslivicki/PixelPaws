"""
Paw Contour graph views  (gait_views.py)
========================================
Headless-of-Tk-state port of the old Gait & Limb "graph window"
(gait_limb_tab._open_graphs and its builders), cut down to the paw-contour
and ROI-brightness graphs (the gait, limb-use and movement categories were
removed 2026-09-18; their columns in old session bundles are ignored). The
Paw Contour tab drives this module through:
...
# One implementation, shared with the engine.
from gait_core import shape_metrics_batch as _shape_metrics_batch  # noqa: E402
...
def _recompute_shape_ratios(frame, row):
    """Filtered circularity / solidity ratios for one row of the summary or bins table,
    after the Filter Preview changed which contours count."""
    import gait_core as gc
    for hl, hr, out in gc.SHAPE_RATIO_COLUMNS:
        if not out.startswith('pawlike_') or hl not in frame.columns or hr not in frame.columns:
            continue
        vHL, vHR = frame.at[row, hl], frame.at[row, hr]
        ok = not (np.isnan(vHL) or np.isnan(vHR)) and vHR > 0
        frame.at[row, out] = round(vHL / vHR, 4) if ok else float('nan')
...
def _add_stat_annotation(host, ax, groups: dict, y_top: float):
    ...
    try:
        ...
        print(f"Warning: statistical test failed in paw contour graph: {_stats_err}")
...
def _analyzed_mask(inter, n_frames):
    """The run's frame selection (licking exclusion, optional 4-paw gate),
    from the same gait_core helper the engine uses, so re-filtering keeps
    the frames the original run analyzed."""
    import gait_core as gc
    params = inter.get('params') or {}
    am, _base, _four = gc.compute_selection_masks(
        inter.get('contact_masks', {}), inter.get('lick_mask'),
        bool(params.get('gate_4paw')), n_frames)
    return am


def _and_analyzed(mask, am):
    k = min(len(mask), len(am))
    return np.asarray(mask[:k], dtype=bool) & am[:k]
...
def recompute_pawlike_metrics(host):
    ...
    over the contour-gated frames the run analyzed (licking excluded)."""
    ...
    for idx, row in host.summary_df.iterrows():
        ...
        am = _analyzed_mask(inter, n_frames)
        ...
        for role in list(pcd.keys()):
            ...
            mask_arr = _and_analyzed(mask_arr, am)
        ...
        _recompute_shape_ratios(df, idx)
    ...
    if host.bins_df is not None and not host.bins_df.empty:
        for bidx, brow in host.bins_df.iterrows():
            ...
            for role in list(pcd.keys()):
                ...
                mask_arr = _and_analyzed(mask_full, am)[frame_slice]
            ...
            _recompute_shape_ratios(bdf, bidx)
...
def create_wb_statistics_tab(host, frame, summary_df, bins_df,
    ...
    # Metrics grouped by category (paw-contour shape + ROI brightness only;
    # gait / movement columns of an old session bundle are never listed).
    contour_metrics = ['paw_area_ratio_hind', 'contact_intensity_ratio_hind',
                       'paw_circularity_ratio_hind', 'paw_solidity_ratio_hind']
    for mk in ('paw_area', 'contact_intensity', 'paw_spread', 'paw_width',
               'paw_solidity', 'paw_aspect_ratio', 'paw_circularity'):
        contour_metrics += [f'{mk}_{r}' for r in ('HL', 'HR')]
    ...
    _stats_categories = [
        ...
        ('ROI Brightness', brightness_metrics),
    ]
...
def export_wb_statistics(host, stats_data, path=None, parent=None):
    """Export the statistics summary to CSV. When *path* is None a
...
def build_registry(host) -> 'OrderedDict[str, list]':
    ...
    Ports the paw-contour and brightness registrations of
    gait_limb_tab._open_graphs (:7100-:7952), flattened from the old
    group→category→metric notebooks to one category level driven by two
    comboboxes. Column-presence gates, display names, references, y-labels,
    and descriptions are verbatim. Gait, limb-use and movement columns (still
    present in old session bundles) register nothing.
    """
    df = host.summary_df
    bdf = (host.bins_df
           if host.bins_df is not None and not host.bins_df.empty
    ...
    # 'weight_bearing' is the pre-2026-09-18 key of the brightness graphs.
    show_brightness = _sets.get('brightness', _sets.get('weight_bearing', True))
    ...
    # ── ROI Brightness ────────────────────────────────────────────────────
    # Mean ROI brightness over the contour-gated, analyzed frames.  (The old
    # Contact % graphs, Hind/Fore ratio and the Limb Use / Gait / Movement /
    # Coordination categories were removed with the gait metrics; an old
    # session bundle that still carries those columns simply shows none.)
    if show_brightness:
        cat = _cat('ROI Brightness')
        # Stored as literal HL/HR; flip=True relabels (and inverts when the
        # injured paw is HR) so it reads injured/contralateral like the
        # area / intensity ratio graphs.
        if ('brightness_ratio_HL_HR' in df.columns
            ...
            _reg(cat, 'Brightness Ratio', 'bar', df,
                 ...
                 y_label=f'Brightness ratio {inj}/{contra} (contact frames)',
                 description=f'{inj}/{contra} (injured/contralateral) '
                             'brightness ratio. Reference 1.0 = equal.',
                 flip=True)
        if bdf is not None:
            ...
            if ('brightness_ratio_HL_HR' in bdf.columns
                    and bdf['brightness_ratio_HL_HR'].notna().any()):
                _reg(cat, 'Brightness Ratio - TC', 'timecourse', bdf,
                     'brightness_ratio_HL_HR', reference=1.0,
                     y_label=f'Brightness ratio {inj}/{contra}',
                     description=f'{inj}/{contra} brightness ratio across '
                                 'time bins. Reference 1.0 = equal.',
                     flip=True)
        if not cat:
            del registry['ROI Brightness']
    ...
    _contour_descs = {
        'Area':         ('Mean paw contour area (px²) during contact frames. '
                         'A larger area may indicate more of the paw in contact with '
                         'the floor.'),
        'Spread':       ('Maximum dimension (px) of paw contour bounding box. '
                         'A larger spread may indicate more toe spreading or a flatter paw placement.'),
        'Intensity':    ('Mean pixel brightness within paw contour shape during contact. '
                         'A brighter paw print may indicate closer contact with the floor.'),
        ...
        'Solidity':     ('Solidity of paw contour - ratio of contour area to convex hull area. '
                         'Values near 1.0 indicate a solid, compact paw print; lower values indicate '
                         'an irregular or fragmented print.'),
    ...
    _LICK_NOTE = ('Licking frames are left out automatically: a paw held '
                  'up to the mouth still looks like a paw on the floor.')
    _shape_ratio_descs = {
        'Circularity': ('circularity ratio (4π × area ÷ perimeter²). Above 1.0 = the '
                        'injured paw print is rounder. '),
        'Solidity':    ('solidity ratio (contour area ÷ convex hull area). Above 1.0 = '
                        'the injured paw print is more compact, with fewer '
                        'indentations in its outline. '),
    }
    ...
    def _add_contour_variant(cat_prefix, metrics_list, stance_suffix,
        ...
        # Ratios
        has_ratios = False
        for rk in [ratio_key, intensity_ratio_key, *[k for k, _ in shape_ratio_keys]]:
        ...
        if has_ratios:
            ...
            # Intensity first: it is the headline measure and the graph the
            # tab opens on (this category is moved to the front below).
            if (intensity_ratio_key in df.columns
                ...
                _reg(cat, 'Intensity Ratio Hind', 'bar',
                     ...
                     description=f'{inj}/{contra} (injured/contralateral) '
                                 'contact intensity ratio. Below 1.0 = the '
                                 'injured paw appears less intense, which may '
                                 'indicate less contact with the floor. '
                                 + _LICK_NOTE,
                     flip=True)
            if ratio_key in df.columns and df[ratio_key].notna().any():
                _reg(cat, 'Area Ratio Hind', 'bar',
                     ...
                     description=f'{inj}/{contra} (injured/contralateral) '
                                 'paw area ratio. Below 1.0 = the injured paw '
                                 'print is smaller, which may indicate less '
                                 'contact with the floor. ' + _LICK_NOTE,
                     flip=True)
            # Shape ratios: the two the manuscript reports beside intensity and area.
            for sk, shape in shape_ratio_keys:
                if sk in df.columns and df[sk].notna().any():
                    _reg(cat, f'{shape} Ratio Hind', 'bar',
                         df, sk, reference=1.0,
                         y_label=f'{shape} ratio {inj}/{contra}{ylbl_sfx}',
                         description=f'{inj}/{contra} (injured/contralateral) '
                                     + _shape_ratio_descs[shape] + _LICK_NOTE,
                         flip=True)
            if bdf is not None:
                ...
                if (ratio_key in bdf.columns
                    ...
                    _reg(cat, 'Area Ratio - TC', 'timecourse',
                         ...
                         flip=True)
                for sk, shape in shape_ratio_keys:
                    if sk in bdf.columns and bdf[sk].notna().any():
                        _reg(cat, f'{shape} Ratio - TC', 'timecourse',
                             bdf, sk, reference=1.0,
                             y_label=f'{shape} ratio {inj}/{contra}{ylbl_sfx}',
                             description=f'{inj}/{contra} {shape.lower()} ratio '
                                         'across time bins. ' + _LICK_NOTE,
                             flip=True)
            if not cat:
    ...
    if show_contour and (has_contour or has_stance_contour or has_pawlike_contour):
        ...
        if has_contour:
            _add_contour_variant(
                ...
                shape_ratio_keys=(('paw_circularity_ratio_hind', 'Circularity'),
                                  ('paw_solidity_ratio_hind', 'Solidity')))
        ...
        if has_stance_contour and _sets.get('full_stance', False):
            _add_contour_variant(
                ...
                shape_ratio_keys=(('paw_circularity_ratio_stance_hind', 'Circularity'),
                                  ('paw_solidity_ratio_stance_hind', 'Solidity')))
        ...
        if has_pawlike_contour:
            _add_contour_variant(
                ...
                filter_paw=True,
                shape_ratio_keys=(('pawlike_circularity_ratio_hind', 'Circularity'),
                                  ('pawlike_solidity_ratio_hind', 'Solidity')))
    ...
    # The tab opens on the first category: the injured/contralateral ratios,
    # intensity first.
    if 'Paw Contour - Ratios' in registry:
        registry.move_to_end('Paw Contour - Ratios', last=False)
