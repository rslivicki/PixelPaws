...
def crop_video_ffmpeg(input_path: str, output_path: str,
    ...
    if end_time is not None:
        # -ss before -i resets timestamps to the seek point, so an output
        # -to counted from there (a 10:00-15:00 trim gave 10:00-25:00).
        # Pass the span as a duration instead.
        span = end_time - (start_time or 0.0)
        if span <= 0:
            raise ValueError(f"End time ({end_time}s) must be after start time "
                             f"({start_time or 0}s).")
        cmd += ["-t", str(span)]
    cmd += [
        "-vf", f"crop={w}:{h}:{x}:{y}",
        ...
        "-preset", "ultrafast",
        "-an",
        # Keep PawCapture's calibration tags (mm_per_pixel,
        # pixelpaws_calibrated, pixelpaws_ref_*); ffmpeg drops custom MP4
        # tags without these two flags.
        "-map_metadata", "0",
