...
def open_diagnose_dialog(parent, project_folder: str) -> None:
    ...
    def worker() -> None:
        try:
            ...
            import sys
            sys.path.insert(0, str(Path(__file__).resolve().parent.parent / 'scripts' / 'utilities'))
            try:
...
def open_normalize_dialog(parent, project_folder: str) -> None:
    ...
    # Blank = each video at its content rate (stored fps x unique-frame
    # fraction), which keeps durations. A fixed default (was 30) stretched
    # any video whose content rate differed.
    fps_var = tk.StringVar(value='')
    ttk.Entry(opts, textvariable=fps_var, width=8).grid(row=0, column=1, sticky='w', padx=4)
    ttk.Label(opts, text='blank = per-video content rate (keeps duration)',
    ...
    def warn_fps(report, dry_run: bool) -> None:
        """Pop up the videos whose explicit fps is >5% off their content rate."""
        off = [p for p in report.plans if p.fps_warning]
        if not off:
            return
        lines = [f'{Path(p.video).name}: {p.fps_warning}' for p in off[:12]]
        if len(off) > 12:
            lines.append(f'... and {len(off) - 12} more (see the log)')
        head = ('Applying with this target fps would change these videos\' durations. '
                'Leave the fps blank to use each video\'s content rate.'
                if dry_run else
                'These videos were written at an fps that changes their duration '
                '(originals are in _pre_fps_normalize_<ts>/).')
        messagebox.showwarning('Normalize Frame Rate', head + '\n\n' + '\n\n'.join(lines),
                               parent=win)

    def worker(target_fps, eps: float, dry_run: bool) -> None:
        try:
            ...
            win.after(0, warn_fps, report, dry_run)
        except Exception:
            win.after(0, append, '\n! Normalize failed:')
            win.after(0, append, traceback.format_exc())
        finally:
    ...
    def start():
        try:
            ...
            tfps = float(fps_txt) if fps_txt else None
            eps = float(eps_var.get().strip())
        except ValueError:
            messagebox.showerror('Normalize Frame Rate',
                                 'target fps (or blank) and eps must be numbers.')
            return
        if tfps is not None and tfps <= 0:
            messagebox.showerror('Normalize Frame Rate',
                                 'target fps must be positive (or blank).')
            return

        will_apply = apply_var.get()
        if will_apply:
            fps_note = ('Each video is written at its own content rate.\n\n'
                        if tfps is None else
                        f'Every video is written at {tfps:g} fps; any whose content '
                        f'rate differs by more than 5% changes duration (a dry run '
                        f'lists them).\n\n')
