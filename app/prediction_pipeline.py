...
def smooth_with_exclusions(y_proba, clf_data, mode='bout_filters', exclude=None):
    """apply_smoothing for a behavior under an exclusion rule.

    ``exclude`` marks frames where an overriding behavior fired (e.g. licking
    over grooming). They are zeroed in the probabilities before bout filtering
    AND in the final calls: zeroing the probabilities alone is not enough,
    because gap bridging (max_gap) joins the bouts on either side of a short
    overriding bout and refills its frames.

    Returns (y_proba with excluded frames zeroed, 0/1 calls).
    """
    y_proba = np.asarray(y_proba, dtype=float)
    if exclude is None:
        return y_proba, apply_smoothing(y_proba, clf_data, mode)
    m = np.zeros(len(y_proba), dtype=bool)
    ex = np.asarray(exclude, dtype=bool)
    n = min(len(ex), len(y_proba))
    m[:n] = ex[:n]
    if not m.any():
        return y_proba, apply_smoothing(y_proba, clf_data, mode)
    y_proba = y_proba.copy()
    y_proba[m] = 0.0
    y_pred = np.asarray(apply_smoothing(y_proba, clf_data, mode)).copy()
    y_pred[m] = 0
...
def predict_with_xgboost(model, X, calibrator=None, fold_models=None,
    ...
    This lets classifiers trained on a subset of the features run, and prevents feature mismatch errors.
