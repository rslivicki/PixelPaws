...
class LocomotionTab(ttk.Frame):
    ...
    def scan_sessions(self):
        ...
        for ext in _VIDEO_EXTS:
            for vp in sorted(_glob.glob(os.path.join(vd, f"*{ext}"))):
                ...
                h5s = rank_h5(_glob.glob(os.path.join(vd, f"{stem}DLC*.h5")))
                if not h5s:
                    continue
                ...
                if read_calibration is not None:
                    try:
                        ...
                        pass
                # rank_h5 already put the active-scorer / newest file first;
                # re-sorting by name would let an old network's file win.
                self._sessions[stem] = {"video": vp, "h5": h5s[0],
                                        "fps": fps, "mm": mm,
                                        "w": fw, "h": fh}
    ...
    def _stats_text(self):
        ...
        # Denominator = the duration the whole bins actually cover (_binned
        # drops the partial last bin), so velocity agrees with the per-bin
        # view instead of being diluted by the uncounted tail.
        dur = {}
        for s_, v in per_sess.items():
            fps = self._sessions[s_]["fps"]
            bin_frames = max(int(round(bin_min * 60 * fps)), 1)
            dur[s_] = len(v) * bin_frames / fps
