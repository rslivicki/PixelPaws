...
def _render_clip(cap, data, body_parts, colors, args,
    ...
    for frame_idx in range(start_frame, end_frame):
        ...
        for bp in body_parts:
            ...
            likelihoods[bp] = lk
            # NaN likelihood compares False against the threshold, so test
            # finiteness explicitly; NaN x/y would crash int(round(...)).
            if not np.isfinite(lk) or lk < args.likelihood:
                continue
            x_raw = float(xs[frame_idx])
            y_raw = float(ys[frame_idx])
            if not (np.isfinite(x_raw) and np.isfinite(y_raw)):
                continue
            # Raw coordinates → canvas coordinates (subtract crop offset)
            cx = int(round(x_raw)) - cx1
            cy = int(round(y_raw)) - cy1
