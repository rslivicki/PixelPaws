...
pp_runtime.resolve_pose_python() and parsing its stdout progress markers. Model
...
class DLCRunDialog(tk.Toplevel):
    ...
    def __init__(self, root, project_folder: str, on_add_videos=None):
        ...
        try:
            ...
            # stale active_bundle.txt or a bundle folder deleted by hand
            self.bundle = None
            _bundle_err = f"\n\n{e}"
        if self.bundle is None:
            messagebox.showerror(
                ...
                "could not be found or loaded." + _bundle_err,
                parent=root,
            )
            self.result = None
            self.destroy()
        ...
        ttk.Checkbutton(
            cfg, text="Run Paw Contour analysis after the classifiers "
        ...
        ttk.Checkbutton(
            cfg, text="Select more frames to label after pose tracking",
            variable=self.frames_var,
        ).grid(row=4, column=0, columnspan=3, sticky="w", pady=(2, 0))

        # ffprobe is what judges the encode (codec detection, output check);
        # ffmpeg without it would make every transcode look failed.
        ...
        if self._ffmpeg_ok:
            ...
            _tr_hint = ("ffmpeg/ffprobe were not found on PATH; install ffmpeg "
                        "(which ships ffprobe) to enable transcoding.")
...
class DLCProgressDialog(tk.Toplevel):
    ...
    def __init__(self, root, settings: dict,
        ...
        self._procs: List[subprocess.Popen] = []      # concurrent transcodes
    ...
    def _video_codec(path) -> str:
        # shared with the Add-videos import (pp_runtime.video_codec)
        try:
            from pp_runtime import video_codec
    ...
    @staticmethod
    def transcode_workers() -> int:
        """How many encodes to run at once. x265 keeps about 4 frame threads busy at
        box-video frame sizes (712x748: 4 frame threads, 12 WPP rows), a quarter of a
        modern CPU; running several files at once uses the rest. PP_TRANSCODE_JOBS
        overrides."""
        try:
            n = int(os.environ.get("PP_TRANSCODE_JOBS", "0"))
            if n > 0:
                return n
        except ValueError:
            pass
    ...
    def _transcode_videos(self, videos):
        """The intake encode (pp_runtime.build_transcode_cmd) applied
        in place: the transcoded file keeps the video's name so downstream
        pairing is unchanged; the original moves to videos/raw/. Several
        files encode at once (transcode_workers). Returns the (possibly
        updated) video list in the input order, or None if cancelled."""
        try:
        ...
        for i, vp in enumerate(videos):
            ...
            if raw.exists():
                if src.exists() and transcode_output_ok(src):
                    self._msg_queue.put(("log", f"\u21b7 {src.name}: raw copy already in "
                                                "videos/raw/ and output verified - "
                                                "already transcoded"))
                    out[i] = str(src)
                    continue
                # An earlier run died mid-encode: discard the partial output,
                # put the original back and encode again.
                self._msg_queue.put(("log", f"\u27f3 {src.name}: raw copy in videos/raw/ "
                                            "but the transcoded file is missing or "
                                            "invalid - re-encoding"))
                try:
                    if src.exists():
                        src.unlink()
                    ...
                    self._msg_queue.put(("log", f"\u2717 {src.name}: could not restore the "
                                                f"original from videos/raw/ ({e}) - "
                                                "tracking what is there"))
                    out[i] = str(src if src.exists() else raw)
                    continue
            todo.append((i, src))
        if not todo:
            return out

        totals = {i: self._frame_count(src) for i, src in todo}
        grand = sum(totals.values())
        done = {i: 0 for i, _ in todo}
        finished = []
        lock = threading.Lock()
        t_batch = _time.time()

        def _progress(i, frames):
            with lock:
                done[i] = frames
                d_all = sum(done.values())
                elapsed = max(_time.time() - t_batch, 1e-3)
            if grand:
                self._msg_queue.put(("frames", (min(d_all, grand), grand, d_all / elapsed)))
        ...
        def _encode(i, src):
            ...
            raw = raw_dir / src.name
            with lock:
                k = len(finished) + sum(1 for j in done if done[j] and j not in finished) + 1
            self._msg_queue.put(("video_start", (min(k, len(todo)), len(todo),
                                                 f"transcode {src.name}", grand)))
            ...
            for line in proc.stdout:
                ...
                if line.startswith("frame="):
                    try:
                        _progress(i, int(line.split("=", 1)[1]))
                    except ValueError:
                        continue
            proc.wait()
            with lock:
            ...
            ok = (not self._cancelled and proc.returncode == 0
                  and transcode_output_ok(dst))
            if ok:
                _progress(i, totals[i] or done[i])
        ...
        with ThreadPoolExecutor(max_workers=jobs) as pool:
            results = list(pool.map(lambda t: _encode(*t), todo))
        if self._cancelled:
            return None
    ...
    def _kill_proc_tree(self):
        """Force-kill the current subprocess(es) and all their children."""
        procs = [self.proc] + list(getattr(self, "_procs", []))
        for proc in procs:
    ...
    def _worker(self):
        # Any uncaught error in the worker must still post all_done, or the
        # dialog spins forever with the Quick Start tab polling a dead thread.
        try:
            self._worker_body()
        except Exception as e:
            self._msg_queue.put(("log", f"✗ pose worker failed: {e!r}"))
    ...
    def _worker_body(self):
        ...
        try:
            self.proc = subprocess.Popen(
                ...
                encoding="utf-8", errors="replace",     # child prints UTF-8
    ...
    def _drain_queue(self):
        # Quick Start also calls this once the worker has ended, so it must be safe to run
        # twice and after the window is gone.
        if getattr(self, "_all_done_seen", False):
            return
        try:
            while True:
            ...
            return                                  # the window was closed mid-read
        try:
            self.after(100, self._drain_queue)
        except tk.TclError:
            pass
