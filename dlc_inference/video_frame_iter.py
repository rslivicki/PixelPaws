...
def prefetched_batches(
    ...
    **kwargs,
    ...
    err: list = []
    stop = threading.Event()      # set by the consumer when it bails out early

    def _put(item) -> bool:
        # Bounded put that keeps checking the stop flag: a consumer that
        # raised mid-stream is no longer draining, so a plain q.put() would
        # block forever and leak the VideoCapture.
    ...
    try:
        ...
        stop.set()
        # drain so a worker blocked in put() can see the flag and exit
        try:
            while True:
                q.get_nowait()
        except queue.Empty:
