...
def run_dlc_on_video(
    ...
    # CAP_PROP_FRAME_COUNT is a container estimate: truncated / VFR files
    # decode fewer frames than it says (and a broken file says 0). Rows are
    # NaN until a decoded frame fills them and the arrays are cut down to
    # what was actually decoded before writing.
    total_frames = count_video_frames(str(video_path))
    all_xyl = np.full((max(total_frames, 0), num_keypoints, 3), np.nan, dtype=np.float32)
    ...
    t0 = time.time()
    frames_done = 0
    max_index = -1                      # highest frame index actually decoded
    with torch.inference_mode():
        for batch in prefetched_batches(
            ...
            for k, fidx in enumerate(batch.frame_indices):
                fidx = int(fidx)
                if fidx >= all_xyl.shape[0]:
                    # more frames than the header claimed: grow the buffer
                    extra = np.full((fidx + 1 - all_xyl.shape[0], num_keypoints, 3),
                                    np.nan, dtype=np.float32)
                    all_xyl = np.concatenate([all_xyl, extra], axis=0)
                all_xyl[fidx] = poses[k, 0]
                max_index = max(max_index, fidx)
    ...
    if frames_done == 0 or max_index < 0:
        raise RuntimeError(
            f"no frames could be decoded from {video_path.name} "
            f"(header reports {total_frames}); no pose file written"
        )
    num_frames = max_index + 1
    if num_frames != total_frames:
        print(f"{video_path.name}: header reports {total_frames} frames, "
              f"decoded {num_frames} - writing {num_frames} rows", flush=True)
    all_xyl = all_xyl[:num_frames]
