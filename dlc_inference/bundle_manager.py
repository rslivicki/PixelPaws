...
def install_default_bundle(source_dir: Path) -> str:
    """Copy the shipped bundle dir from `source_dir` into bundles_root().

    First run: the copy populates %LOCALAPPDATA%. Later runs: the copy is refreshed
    when the shipped manifest.json differs from the installed one, so a newer
    network in the app (a new release, or a git pull when running from source)
    reaches the user. The installer does the same comparison; this covers the
    from-source case. The user's own imported bundles (other ids) are untouched.
    """
    src = Path(source_dir)
    if not src.is_dir():
    ...
    bundle_id = json.loads(manifest.read_text(encoding="utf-8"))["bundle_id"]
    dest = bundles_root() / bundle_id
    if dest.is_dir():
        installed = dest / "manifest.json"
        if installed.is_file() and installed.read_bytes() == manifest.read_bytes():
            return bundle_id
        shutil.rmtree(dest)
...
def import_bundle_zip(zip_path: Path, overwrite: bool = False) -> str:
    ...
    with zipfile.ZipFile(zp) as zf:
        ...
        if dest.is_dir():
            ...
            shutil.rmtree(dest)
        # The zip's top-level folder need not be named after bundle_id, so
        # extract to a scratch dir and move the folder holding manifest.json
        # into place (extracting straight into bundles_root() would leave
        # <bundle_id>/ empty whenever the names differ).
        import tempfile
        tmp = Path(tempfile.mkdtemp(prefix=".import_", dir=str(dest.parent)))
        try:
            zf.extractall(tmp)
            src = tmp / Path(manifest_member).parent
