...
def _template_rank(name):
    ...
    for i, tok in enumerate(_PRIORITY_TEMPLATE):
        if tok in n:
            return i
    return len(_PRIORITY_TEMPLATE) - 2.5          # unknown: above moving/still
...
class SequencingTab(ttk.Frame):
    ...
    def __init__(self, parent, main_gui):
        ...
        self.app = main_gui
        self._cohort = None
        self._cohort_bouts = {}   # {session: bout count} for the cohort's sessions
        self._groups = []
    ...
    def _subject_of(self, session):
        """Match a session stem to a key-file Subject: exact, else the Subject
        must be a whole underscore-separated token of the stem ('mouse1'
        matches 'mouse1_veh' but never 'mouse10_veh'; no substring fallback,
        so integer subjects 1..8 do not all collapse onto '1')."""
        from ui_session_filter import _match_key_row
        if self._key_df is None:
            return None
        subs = [str(s).strip() for s in self._key_df["Subject"]]
        if session in subs:
            return session
        row = _match_key_row(self._key_df, session)
        return str(row["Subject"]).strip() if row is not None else None
    ...
    def compute(self):
        ...
        # Bouts per included session (the cohort's own bout definition) for the
        # Σ Stats per-group summary - SyntaxCohort keeps only the transition tables.
        self._cohort_bouts = {s: int(SA.bout_sequence(seqs[s], keep_idx).size)
                              for s in self._cohort.sessions}
        if not self._cohort.sessions:
            ...
            self._status.set(
                ...
                f"(bouts)' in Display thresholds (e.g. {max(most // 2, 5)}) "
                f"and Compute again.")
            return
    ...
    def _stats_text(self):
        ...
        try:
            import numpy as _np
            # SyntaxCohort stores sessions + their groups (not seqs/group_of);
            # bout counts were kept at Compute time.
            group_of = dict(zip(c.sessions, map(str, c.groups_per_session)))
            for g in self._groups:
                subs = [s for s in c.sessions if group_of.get(s) == str(g)]
                bouts = [self._cohort_bouts[s] for s in subs
                         if s in self._cohort_bouts]
                if bouts:
                    lines.append(f"  {g}: {len(subs)} animal(s), "
                                 f"median bouts {_np.median(bouts):g} "
                                 f"(range {min(bouts)}-{max(bouts)})")
        except Exception as e:
            lines.append(f"(per-group bout summary unavailable: {e})")
        lines.append("")
