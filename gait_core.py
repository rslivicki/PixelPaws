"""
gait_core.py - headless Paw Contour analysis compute
====================================================
...
``compute_gait`` (optional, default False) gates every gait and movement
metric: per-paw contact % and the WBI/SI/SBI indices built on it, stance /
swing / stride timing, stride and step geometry, symmetry indices, phase
coupling, regularity index, print position, support patterns, the whole
``loco_`` block, distance, body speed and time moving (see
``is_gait_column``).  The Paw Contour tab leaves it off, so its summary and
bins carry only the contour-shape and ROI-brightness metrics plus
``contour_pass_pct`` (the share of analyzed frames where the hind-paw
contours pass the area band).  Scripts that still want the old outputs pass
``compute_gait=True``.
"""
...
# user never has to know that contour needs brightness needs video.  Contour
# area is the only contact method the tab runs, so every preset selects it.
GAIT_PRESETS = {
    ...
    # licking excluded, 5-min bins. No DLC-likelihood frame filter: its
    ...
    'Paw contour (manuscript gate)': dict(
        brightness=True, contour=True, fore=False, contour_roi=50,
        bin_seconds=5, bin_unit='minutes',
        contact_method='contour_area', contour_area_threshold=1500,
        contour_area_max=5000, exclude_licking=True),
    'Contour + brightness':          dict(brightness=True,  contour=True, fore=False,
                                          contact_method='contour_area'),
    'Contour, no brightness':        dict(brightness=False, contour=True, fore=False,
                                          contact_method='contour_area'),
    'Contour + fore paws':           dict(brightness=True,  contour=True, fore=True,
                                          contact_method='contour_area'),
}

# Preset names saved by earlier versions (the gait presets) -> the preset that
# sets the same toggles now, so an old session bundle restores a listed name.
LEGACY_PRESET_NAMES = {
    'Hindpaw gait (minimal)':      'Contour, no brightness',
    'Gait + forepaws':             'Contour + fore paws',
    'Gait + brightness':           'Contour + brightness',
    'Gait + brightness + contour': 'Contour + brightness',
}


# ─────────────────────────────────────────────────────────────────────────────
# Gait / movement columns (emitted only with params['compute_gait'])
# ─────────────────────────────────────────────────────────────────────────────

#: Column-name prefixes of the gait and movement metrics.
GAIT_COLUMN_PREFIXES = (
    'loco_', 'contact_pct_', 'n_contact_', 'WBI_', 'SI_', 'SBI_',
    'stance_dur_', 'swing_dur_', 'stride_dur_', 'duty_cycle_', 'cadence_',
    'n_strides_', 'stride_len_', 'swing_speed_', 'stride_cv_',
    'step_len_', 'step_width_', 'phase_', 'print_position_', 'support_',
    'body_speed_', 'time_moving_',
)
#: Gait and movement columns without one of the prefixes above.
GAIT_COLUMNS = frozenset({
    'hind_fore_ratio', 'regularity_index', 'total_distance',
    'stance_SI_hind', 'quad_stance_pct',
})


def is_gait_column(name):
    """True for a gait / movement metric column (one that the Paw Contour
    analysis no longer reports; ``compute_gait=True`` still emits them)."""
    name = str(name)
    return name in GAIT_COLUMNS or name.startswith(GAIT_COLUMN_PREFIXES)


def drop_gait_columns(df):
    """``df`` without its gait / movement columns (None passes through)."""
    if df is None:
        return None
    gait = [c for c in df.columns if is_gait_column(c)]
    return df.drop(columns=gait) if gait else df
...
class GaitContext:
    ...
    project_folder : str
        ...
        the cache dirs (``analysis_dir()``: ``paw_contour/``, or
        ``gait_limb_analysis/`` where a project already has it; legacy
...
SKIP_NO_CONTOUR = "no contour data (no video?) - session skipped"


def _record_skip(skip_reasons, session_name, reason):
    """Note why a session produced no row (``skip_reasons`` may be None)."""
    if skip_reasons is not None:
        skip_reasons[session_name] = reason
...
def load_lick_mask(folder, session_name, behavior, thr, n_frames):
    ...
    if vals is None:
        ...
        # The stem must be followed by '_' (or be the whole name): a bare
        # f'{stem}*' prefix glob pairs ..._S1 with ..._S10's predictions
        # (same rule as h5_resolve / scoring_preview).
        cands = []
        for pat in (f'{session_name}_*_predictions.csv',
                    f'{session_name}_predictions.csv'):
            cands += _glob.glob(os.path.join(results, behavior, pat))
            cands += [p for p in _glob.glob(os.path.join(
                results, '**', pat), recursive=True)
...
#: Folder (inside the project) for Paw Contour caches and session bundles.
ANALYSIS_DIR = 'paw_contour'
#: Older names. A project that already has gait_limb_analysis/ (and no
#: paw_contour/) keeps using it, so scripts reading those folders keep
#: working; weight_bearing_analysis/ is only ever read (cache files).
LEGACY_ANALYSIS_DIRS = ('gait_limb_analysis', 'weight_bearing_analysis')


def analysis_dir(project_folder):
    """Paw Contour folder for ``project_folder``: paw_contour/ for new
    projects, gait_limb_analysis/ for projects that already use it."""
    new = os.path.join(project_folder, ANALYSIS_DIR)
    if os.path.isdir(new):
        return new
    old = os.path.join(project_folder, LEGACY_ANALYSIS_DIRS[0])
...
def gait_cycles(mask, fps, min_stride_ms=0, valid=None):
    """Stance / swing bouts and complete strides of one paw's contact mask.

    Bouts are found on ``mask`` exactly as gait_bouts finds them (stances
    shorter than ``min_stride_ms`` are dropped as noise and the swing spans
    the gap between the retained stances).  ``valid`` (optional per-frame
    bool, e.g. confidence & locomotion gates) is applied AFTER bout
    detection: a stance or swing bout that overlaps any invalid frame is
    dropped.  (AND-ing the gate into the mask turned an invalid stretch
    inside one stance into two short stances plus a fake swing.)

    A stride is a retained stance plus the swing that follows it, up to the
    next stance onset; both must be valid.

    Returns dict:
        stance_durs   : list[float] - valid stance bouts (s)
        swing_durs    : list[float] - valid swing bouts (s)
        stance_onsets : list[int]   - onset frame of each valid stance
        strides       : list[(stance_s, swing_s, onset, next_onset)] -
                        complete valid strides, in onset order
    With ``valid=None`` the first three equal gait_bouts' output.
    """
    out = {'stance_durs': [], 'swing_durs': [], 'stance_onsets': [],
           'strides': []}
    m = np.asarray(mask, dtype=bool)
    n = len(m)
    if n == 0:
    ...
    if valid is not None:
        ...
        if len(ok) != n or ok.all():
            ok = None

    def _valid(a, b):
        return ok is None or bool(ok[a:b].all())

    d = np.diff(m.astype(int), prepend=int(~m[0]))
    onsets = np.where(d == 1)[0]     # swing→stance transitions
    offsets = np.where(d == -1)[0]   # stance→swing transitions
    if len(onsets) == 0:
        # all swing (an all-stance mask has an onset at frame 0)
        if _valid(0, n):
            out['swing_durs'].append(n / fps)
        return out

    min_frames = (max(1, round(min_stride_ms / 1000.0 * fps))
                  if min_stride_ms > 0 else 0)
    runs = []                        # retained stance bouts [on, off)
    for on in onsets:
        idx = np.searchsorted(offsets, on, side='right')
        off = int(offsets[idx]) if idx < len(offsets) else n
        if min_frames > 0 and off - on < min_frames:
            continue
        runs.append((int(on), off))

    for i, (on, off) in enumerate(runs):
        stance_ok = _valid(on, off)
        if stance_ok:
            out['stance_durs'].append((off - on) / fps)
            out['stance_onsets'].append(on)
        if i + 1 < len(runs):
            nxt = runs[i + 1][0]
            if nxt - off > 0:
                swing_ok = _valid(off, nxt)
                if swing_ok:
                    out['swing_durs'].append((nxt - off) / fps)
                if stance_ok and swing_ok:
                    out['strides'].append(((off - on) / fps,
                                           (nxt - off) / fps, on, nxt))
    return out
...
#: (HL column, HR column, ratio column) for the shape ratios, per contour variant:
#: all gated frames, full stance, and the Filter Preview's filtered set.
SHAPE_RATIO_COLUMNS = tuple(
    (hl, hr, out)
    for shape in ('circularity', 'solidity')
    for hl, hr, out in (
        (f'paw_{shape}_HL', f'paw_{shape}_HR', f'paw_{shape}_ratio_hind'),
        (f'paw_{shape}_stance_HL', f'paw_{shape}_stance_HR', f'paw_{shape}_ratio_stance_hind'),
        (f'pawlike_{shape}_HL', f'pawlike_{shape}_HR', f'pawlike_{shape}_ratio_hind'),
    )
)
...
def load_session_data(sess, paw_map, params, ctx,
    ...
    if not dlc_file or not os.path.isfile(dlc_file):
        ...
        _record_skip(skip_reasons, sess['session_name'], "no DLC file")
        return None
    ...
    if _mm_per_px is not None:
        log(f"  ✓ Calibration on: pose coordinates in mm "
    ...
    try:
        ...
        log(f"  DLC error: {e}")
        _record_skip(skip_reasons, sess['session_name'], f"DLC error: {e}")
        return None
    ...
    # The DLC confidence mask and the body-speed / locomotion arrays only feed
    # the gait and movement metrics, so they are skipped unless requested.
    _compute_gait = bool(params.get('compute_gait', False))

    # ── DLC confidence mask (Step 6) ─────────────────────────────────────
    confidence_mask = None
    if _compute_gait and params.get('use_likelihood') and bp_prob is not None:
    ...
    # ── Locomotion filter mask & body speed (with compute_gait) ───────
    loco_mask = None
    body_speed = None
    ...
    for candidate in ['tailbase', 'tail_base', 'tb']:
        ...
        if tb_x_col:
            tb_bp = candidate
            break
    if not _compute_gait:
        cx = cy = None
    elif tb_x_col is None:
    ...
    for role, bp in active_paws.items():
        ...
        if px is not None:
            paw_xy[role] = (px, py)
    # Contour boxes are cut out of video pixels. With calibration on, the pose
    # coordinates above are in mm (pose_features multiplies by mm_per_pixel),
    # and placing boxes with them put every box near the image origin: no
    # contour at all on some sessions, background on others, no error.
    if _mm_per_px:
        paw_xy_px = {r: (x / float(_mm_per_px), y / float(_mm_per_px))
                     for r, (x, y) in paw_xy.items()}
    else:
        paw_xy_px = paw_xy
    ...
    if contour_paws and video_file and os.path.isfile(video_file) and _CV2_OK:
        ...
        contour_cache_key = {
            ...
            # The brightness pass (which the contour extraction piggy-backs
            # on) can be restricted to height-contact frames, so caches
            # written under a different contact gate are not interchangeable.
            'brt_weight': params.get('brt_weight', 1.0),
            'contact_threshold': params.get('contact_threshold', 15),
        }
        if _mm_per_px:
            # Calibrated sessions placed boxes in mm before 2026-09-18; the
            # extra field keeps those misplaced caches from being reused.
            # Uncalibrated sessions keep their existing cache names.
            contour_cache_key['roi_coords'] = 'px'
    ...
    if params['use_brightness'] and video_file and os.path.isfile(video_file):
        ...
        if not brightness_series:
            ...
            try:
                ...
                # Never when contour paws are requested: the contour callback
                # rides on this pass and must see every frame, otherwise
                # areas stay 0 outside the height-contact frames and the
                # contour gate can only ever confirm the height gate.
                hint_mask = None
                if (abs(params.get('brt_weight', 1.0)) < 1e-6 and contact_masks
                        and not contour_paws
                        and params.get('contact_method') != 'contour_area'):
                ...
                if (contour_paws and _CV2_OK
                    ...
                    def contour_callback(i_frame, gray_u8, frame):
                        ...
                        for role, bp in contour_paws.items():
                            if role not in paw_xy_px:
                                continue
                            px_arr, py_arr = paw_xy_px[role]
    ...
    # Without contour data the contour gate cannot run; falling back to the
    # height-gated masks would emit a row indistinguishable from the
    # contour-gated ones, so the session is skipped instead.
    if params.get('contact_method') == 'contour_area' and not paw_contour_data:
        log(f"  {SKIP_NO_CONTOUR}")
        _record_skip(skip_reasons, sess['session_name'], SKIP_NO_CONTOUR)
        return None
...
def compute_all_metrics(data, selection_masks, params, frame_slice=None,
    ...
    params : dict - the 33-key params contract (bin_seconds/bin_unit drive
        the per-bin rows; ``compute_gait`` adds the gait / movement metrics).
    ...
    compute_gait = bool(params.get('compute_gait', False))
    # Hind paws whose contact mask IS the contour-area band (both hind paws
    # share one mask when both have contour data).
    contour_gate_roles = (
        [r for r in ('HL', 'HR') if r in contact_masks
         and r in paw_contour_data and 'areas' in paw_contour_data[r]]
        if params.get('contact_method') == 'contour_area' else [])
    ...
    def _metrics(frame_slice=None):
        ...
        # Contour gate pass % (QC, not a behavior): of the licking-excluded
        # frames, the share the contour metrics come from (hind contours in
        # the area band, and all four paws down when the 4-paw gate is on).
        # The denominator is the base mask, not the analyzed mask: the
        # 4-paw mask is built from the same band masks, so over analyzed
        # frames the pass % would read 100 whenever the gate is on.  Gate
        # off (the manuscript preset) it is unchanged and equals the old
        # contact_pct_HL (== contact_pct_HR).
        if contour_gate_roles:
            _pass = None
            for role in contour_gate_roles:
                mask = masks[role]
                marr = (mask.values.astype(bool) if hasattr(mask, 'values')
                        else np.asarray(mask, bool))[:n]
                _pass = marr if _pass is None else (_pass & marr)
            _nb = int(_bm.sum())
            m['contour_pass_pct'] = (
                round(float((_pass & _am).sum()) / _nb * 100, 2)
                if _nb > 0 else float('nan'))
        ...
        if compute_gait and _fm is not None:
            _fm = _fm[:n]
            _bden = int(_bm.sum())
            m['quad_stance_pct'] = (
        ...
        # Hind WBI / SI. Under the contour gate both hind paws share one
        # mask, so these would always read 50 / 0 / 0: reported as NaN.
        _contour_gate = params.get('contact_method') == 'contour_area'
        if compute_gait and 'HL' in masks and 'HR' in masks:
            cHL, cHR = m['contact_pct_HL'], m['contact_pct_HR']
            tot = cHL + cHR
            if _contour_gate:
                m['WBI_hind'] = m['SI_hind'] = float('nan')
            else:
                m['WBI_hind'] = round(cHL / tot * 100, 2) if tot > 0 else float('nan')
                m['SI_hind']  = round((cHL - cHR) / tot * 100, 2) if tot > 0 else float('nan')

        # Fore WBI / SI
        if compute_gait and 'FL' in masks and 'FR' in masks:
        ...
        if compute_gait and 'HL' in masks and 'HR' in masks:
            ...
            tot = cHL + cHR
            m['SBI_hind'] = round(
                2 * abs(cHL - cHR) / tot * 100, 2) if tot > 0 and not _contour_gate else float('nan')
        if compute_gait and 'FL' in masks and 'FR' in masks:
            cFL, cFR = m['contact_pct_FL'], m['contact_pct_FR']
            tot = cFL + cFR
            m['SBI_fore'] = round(
        ...
        # Hind / fore ratio
        if compute_gait and all(r in masks for r in ('HL', 'HR', 'FL', 'FR')):
        ...
        if compute_gait and frame_displacements is not None:
        ...
        # ── Body speed mean ───────────────────────────────────────────
        if compute_gait and body_speed is not None:
        ...
        if compute_gait and len(masks) >= 4 and loco_mask is not None:
            lm_sl = loco_mask[frame_slice] if frame_slice is not None else loco_mask
        ...
        def _gait_block(m, masks, loco_filter_mask, prefix=''):
            ...
            for role, mask in masks.items():
                ...
                # Bouts on the contact mask itself; stances / swings that overlap
                # invalid frames are dropped (never cut into fake bouts).
                _min_stance = params.get('min_stance_ms', 0)
                cyc = gait_cycles(mask_arr, fps, min_stride_ms=_min_stance,
                                  valid=gait_valid)
                stance_durs, swing_durs = cyc['stance_durs'], cyc['swing_durs']
                strides = cyc['strides']
                ...
                # Stride = each stance + the swing that follows it (per stride,
                # not mean(stance) + mean(swing)); duty = stance / stride per
                # stride, cadence = strides per minute of striding.
                if strides:
                    st_arr = np.array([s[0] for s in strides])
                    stride_arr = st_arr + np.array([s[1] for s in strides])
                    stride_dur = float(np.mean(stride_arr))
                    m[f'{prefix}stride_dur_{role}'] = round(stride_dur, 4)
                    m[f'{prefix}duty_cycle_{role}'] = round(
                        float(np.mean(st_arr / stride_arr)) * 100, 2)
                    m[f'{prefix}cadence_{role}'] = round(60.0 / stride_dur, 2)
                else:
                    m[f'{prefix}stride_dur_{role}'] = float('nan')
                    m[f'{prefix}duty_cycle_{role}'] = float('nan')
                    m[f'{prefix}cadence_{role}'] = float('nan')

                # Stride length (distance between the foot-strikes that open
                # and close each complete stride)
                if role in paw_xy and strides:
                    px_arr, py_arr = paw_xy[role]
                    sl_start = (frame_slice.start or 0) if frame_slice is not None else 0
                    dists = []
                    for _st, _sw, on0, on1 in strides:
                        i0, i1 = sl_start + on0, sl_start + on1
                        if i1 < len(px_arr):
                            dists.append(np.sqrt((px_arr[i1] - px_arr[i0])**2 +
                                                 (py_arr[i1] - py_arr[i0])**2))
                    m[f'{prefix}stride_len_{role}'] = (
                        round(float(np.mean(dists)), 2) if dists else float('nan'))
                else:
        ...
        if paw_contour_data:
            ...
            for role in list(paw_contour_data.keys()):
                ...
                # Contour metrics average over contour-gated frames that are
                # also ANALYZED (licking excluded; 4-paw gate when on). The
                # band alone does not remove licking: a paw held up to the
                # mouth still gives a paw-sized contour, and including those
                # frames flipped the area ratio on a licking vehicle mouse
                # (example project mouse1_veh: 1.19 with, 0.95 without).
                _k = min(len(mask_arr), len(_am))
                mask_sl = np.asarray(mask_arr[:_k], dtype=bool) & np.asarray(_am[:_k], dtype=bool)
                ...
                if stance_mask_all is not None:
                    # stance_mask_all is built from ``masks``, which are
                    # already the bin's slice - do not slice it again.
                    stance_sl = stance_mask_all
            ...
            # Shape ratios (circularity, solidity), in the same three variants. These are the
            # two the manuscript reports beside intensity and area (mean HL / mean HR).
            for _hl, _hr, _out in SHAPE_RATIO_COLUMNS:
                if _hl in m and _hr in m:
                    vHL, vHR = m[_hl], m[_hr]
                    if not (np.isnan(vHL) or np.isnan(vHR)) and vHR > 0:
                        m[_out] = round(vHL / vHR, 4)
...
def analyze_session(sess, paw_map, params, ctx,
    ...
    or None on failure. When ``skip_reasons`` (a dict) is given, a skipped
    session's reason is stored under its name.
    ...
    if params.get('gate_4paw'):
        if four_mask is not None:
            log(f"  4-paw gate: {int(four_mask.sum())}/{n_frames} "
                f"frames with all four paws down.")
        else:
...
def run_sessions(sessions, paw_map, params, key_df, ctx,
    ...
    dict(session=…, subject=…, treatment=…). ``skip_reasons`` (dict, optional)
    receives session_name -> why it produced no row.
    """
    log = log or _noop_log
    summary_rows = []
    bin_rows = []
    ...
    for i_sess, sess in enumerate(sessions):
        ...
        try:
            result = analyze_session(sess, paw_map, params, ctx,
                                     ...
                                     cancel=cancel, skip_reasons=skip_reasons)
            if result:
            ...
            log(f"  ERROR: {e}")
            _record_skip(skip_reasons, name, f"error: {e}")
            not_done.append(name)
        if progress_cb is not None:
            try:
