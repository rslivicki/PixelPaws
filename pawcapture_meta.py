...
def find_session_for_video(mp4_path: PathLike) -> Optional[Path]:
    ...
    def _basename(s: str) -> str:
        # Manifests carry the recording PC's absolute Windows path; split on
        # either separator so this also works when read on POSIX.
        return str(s).replace("\\", "/").rstrip("/").rsplit("/", 1)[-1].lower()

    target_name = _basename(target)
    ...
    for cand in sorted(parent.glob("session_*.json")):
        ...
        for cam in sess.get("cameras", []):
            ...
            if not f:
                continue
            try:
                if Path(f).resolve() == Path(target):
                    return cand
            except (OSError, ValueError):
                pass
            # Fallback: the recordings were copied to another machine, so the
            # absolute path never matches; the file name still does.
            if by_name is None and _basename(f) == target_name:
                by_name = cand
    return by_name
