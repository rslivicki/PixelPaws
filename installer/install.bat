...
REM    - For GPU: NVIDIA driver >= 528 (570 for RTX 50-series; NO CUDA Toolkit needed)
...
if /I "!PP_DEST!"=="!PP_SRC!" (
    ...
    if exist "!PP_DEST!\PixelPaws_GUI.py" (
        ...
        REM The example project is shipped data: replace it whole, so outputs
        REM from running an older example never sit beside new clips that
        REM have the same names (the app would call them already analyzed).
        if exist "!PP_DEST!\example_project\" (
            rmdir /S /Q "!PP_DEST!\example_project" 1>>"%LOGFILE%" 2>&1
            call :log "Replacing the example project with this version's copy"
        )
        REM Same for the bundled classifiers: a renamed or retired classifier
        REM (classifier_walking.pkl became classifier_moving.pkl) must not
        REM linger. User classifiers live in %USERPROFILE%\.pixelpaws instead.
        if exist "!PP_DEST!\pixelpaws_global_classifier_encyclopedia\" (
            rmdir /S /Q "!PP_DEST!\pixelpaws_global_classifier_encyclopedia" 1>>"%LOGFILE%" 2>&1
            call :log "Replacing the bundled classifiers with this version's set"
        )
    ) else (
...
REM wmic is absent on Windows 11 24H2+, so ask CIM via PowerShell first and
REM keep the wmic loop only as a fallback for old machines.
powershell -NoProfile -Command "Get-CimInstance Win32_LogicalDisk -Filter 'DriveType=3' | ForEach-Object { '    {0}   {1} GB free' -f $_.DeviceID, [math]::Floor($_.FreeSpace/1GB) }" 2>nul
if errorlevel 1 (
...
REM Delayed expansion (!VAR!) for the paths: an "&" in the chosen install
REM folder (C:\Lab & Data\PixelPaws) would otherwise split these echo lines
REM at parse time and the split script dies with "Could not parse".
...
REM echo( + delayed expansion so an "&" in the install folder cannot split
REM the command and truncate the recorded root.
...
>> "%PS_RUN%" echo ^& cmd.exe /c $env:PP_CMD 2^>^&1 ^| ForEach-Object { $s = if ($_ -is [System.Management.Automation.ErrorRecord]) { $_.Exception.Message } else { [string]$_ }; if ($s.Trim().Length -gt 0) { Write-Host $s; Add-Content -LiteralPath $env:PP_LOGFILE -Value $s } }
...
echo ============================================================
echo.
REM Same echo( + !VAR! form as :log - "%~1" re-expands the raw message, so an
REM "&" or ")" inside it (e.g. from the install path) breaks the line.
>> "%LOGFILE%" echo(!PP_MSG!
pause
