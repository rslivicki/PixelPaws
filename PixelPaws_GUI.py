...
3. Behavior Ethogram Generator - Automated analysis outputs
4. Data Quality Checker - Pre-training validation
...
- cv_tuning.py - Cross-validation helpers for the Train tab
...
# The intake helpers (pp_runtime: transcode / is_hevc) live in pipeline/;
# make it importable from module import, not only after dlc_run_dialog has
# been imported by a project scan.
...
# The app's own modules (tabs, dialogs, features, analysis) live in app/.
...
# Tabs excluded from the public distribution. Discover (unsupervised), Body Contact,
# Transitions, Active Learning and the two legacy tabs are development / lab-internal
# features; set True to restore them in a working build. Their modules stay in the lab
# repo and are left out of the public tree, so every import of them here is guarded.
...
#
# This allows:
# - A classifier trained on 339 features can use the 375-feature cache by selecting its 339 columns
# - A classifier trained on all 375 uses the full cache
...
try:
    ...
    print("Please ensure pose_features.py and brightness_features.py are in the app folder")
...
from ui_utils import ToolTip, bind_mousewheel, FONT_FAMILY
...
class PixelPawsGUI:
    ...
    def __init__(self, root):
        ...
        # more than one panel can bind to the SAME vars.
        self.train_min_label_bout = tk.IntVar(value=0)
        self.train_max_label_bout = tk.IntVar(value=0)

        # ── CV-eligibility: sparse sessions become training-only
        ...
        # Initialize prediction variables (created by create_evaluation_tab and
        # the Make Labeled Video window it builds)
    ...
    def setup_ui(self):
        ...
        self.notebook = SidebarNav(self.root, width=280, groups={
            ...
            # bundled set -> analyze. Classifier development (Train / Evaluate)
            # is demoted to the bottom group -- most users
            # ship with the bundled classifiers and never train.
            "Get Started": ["⚡ Quick Start"],
            "Pose Tracking": ["🦴 Pose Estimation"],
            ...
            "Tools": ["🛠 Tools"],
            "Train & Evaluate": ["🎓 Train Classifier", "📊 Evaluate"]
        ...
        self.create_evaluation_tab()      # also builds Tools > Make Labeled Video
        ...
        # Paw Contour (gait & limb) analysis tab
        if GAIT_LIMB_TAB_AVAILABLE:
            self.wb_tab_frame = ttk.Frame(self.notebook)
            self.notebook.add(self.wb_tab_frame, text="🐾 Paw Contour")
        ...
        if not GAIT_LIMB_TAB_AVAILABLE:
            self.notebook.hide_item("🐾 Paw Contour")
    ...
    def create_menu(self):
        ...
        tools_menu.add_command(label="Review Scoring (calls on video)",
                               command=self.open_scoring_preview)
        tools_menu.add_command(label="Make Labeled Video\u2026",
                               command=self.open_labeled_video_tool)
    ...
    def _check_for_updates(self):
        ...
        from pp_version import __version__ as _CURRENT
        _REPO = "https://api.github.com/repos/rslivicki/PixelPaws/releases/latest"

        # The request (10 s timeout) runs off the main thread so the window
        # does not freeze; the result dialog is scheduled back via _safe_after.
        def _fetch():
            try:
                ...
                def _vt(v):
                    try:
                        ...
                        return None
                # only a NEWER release is an update (an older tag on the
                # releases page, or a dev build, is not)
                _new = (_vt(tag) > _vt(_CURRENT)
                        if _vt(tag) and _vt(_CURRENT) else tag != _CURRENT)
                if tag and _new:
                    title, msg = ("Update Available",
                                  f"A new version is available: v{tag}\n"
                                  f"You are running v{_CURRENT}\n\n"
                                  f"Visit the GitHub releases page to download.")
                else:
                    title, msg = ("Up to Date",
                                  f"You are running the latest version (v{_CURRENT}).")
            except urllib.error.HTTPError as e:
                if e.code == 404:
                    # GitHub returns 404 from /releases/latest when the repo
                    # has no published release yet - not a network problem.
                    title, msg = ("Check for Updates",
                                  f"No release has been published yet.\n"
                                  f"You are running v{_CURRENT}.")
                else:
                    title, msg = ("Check for Updates",
                                  f"GitHub answered HTTP {e.code} - try again later.")
            except Exception:
                title, msg = ("Check for Updates",
                              "Could not reach GitHub.\n"
                              "Check your internet connection and try again.")
            self._safe_after(lambda: messagebox.showinfo(title, msg))
    ...
    def create_training_tab(self):
        ...
        # Pre-extraction (was the Feature Extraction tab): optional, since
        # training extracts whatever is missing on its own.
        _pre = ttk.Frame(feature_frame)
        _pre.grid(row=101, column=0, columnspan=3, sticky='w', pady=(2, 2))
        ttk.Button(_pre, text="⚙️ Pre-extract Features…",
                   command=self.open_feature_extraction_run).pack(side='left')
        ttk.Button(_pre, text="Single video / manual…",
                   command=self.open_feature_extraction).pack(side='left', padx=(6, 0))
        ttk.Label(_pre, text="Optional: cache features for the project's tracked videos ahead "
                             "of training (training extracts anything missing itself).",
        ...
        # Label-bout-length filter - exclude labeled
        ...
        ttk.Label(params_frame,
                 text="Drop too-short/long labeled positive bouts (→ unobserved, "
                      "in-memory only). 0 = off. 5 fr ≈ 0.08 s @60fps.",
        ...
        ttk.Label(params_frame, text="Fits IsotonicRegression on OOF probabilities - improves P=0.5 thresholding",
    ...
    def create_evaluation_tab(self):
        """📊 Evaluate: one tab to score a classifier against human labels.

        The Classifier box (smoothing and threshold / bout-filter overrides
        included) drives both halves: Compare (quick per-session table,
        diagnostic plots, preview and labeled-video export per session) and
        Full evaluation (evaluation_tab.EvaluationTab in shared mode: pooled
        report, confusion matrix, SHAP, cross-validated parameter
        optimization). Merged from the old Predict & Review and Evaluate
        tabs, which scored the same sessions the same way. Single-video
        prediction and labeled-video styling live in Tools > Make Labeled
        Video (_build_labeled_video_window)."""
        ...
        # ── Header ────────────────────────────────────────────────────────
        ttk.Label(scrollable_frame, text="Evaluate",
                  font=(FONT_FAMILY, 13, 'bold')).pack(anchor='w', padx=15, pady=(10, 0))
        ttk.Label(scrollable_frame,
                  text="Score a classifier against human labels. Pick the classifier once: "
                       "Compare gives a quick per-session table, Full evaluation adds a pooled "
                       "report, confusion matrix, SHAP and parameter optimization. Single-video "
                       "prediction and labeled videos: Tools > Make Labeled Video.",
                  foreground='gray', wraplength=700, justify='left').pack(anchor='w', padx=15, pady=(0, 4))

        # === CLASSIFIER (shared by Compare, Full evaluation and the
        #     Make Labeled Video window) ===
        ...
        self._pr_toggle_override = _toggle_override
        ...
        # ====================================================================
        # FULL EVALUATION: pooled report, confusion matrix, SHAP, optimizer
        # ====================================================================
        full_lf = ttk.LabelFrame(
            scrollable_frame,
            text="Full evaluation - pooled report, confusion matrix, SHAP, parameter optimization",
            padding=6)
        full_lf.pack(fill='both', expand=True, padx=15, pady=8)
        if EVALUATION_TAB_AVAILABLE:
            self.evaluation_tab = EvaluationTab(full_lf, self, shared={
                'classifier': self.pred_classifier_path,
                'smoothing': self.pred_smoothing_mode,
                'read_overrides': self._pr_read_overrides,
                'set_overrides': self._pr_set_overrides,
            })
            ...
            ttk.Label(full_lf,
                      text="⚠️  evaluation_tab.py not found.\n\n"
                           "Place evaluation_tab.py in the app folder next to PixelPaws_GUI.py.",
        ...
        self._build_labeled_video_window()
    ...
    def _build_labeled_video_window(self):
        """Tools > Make Labeled Video: score one video with one classifier,
        preview it with the predictions, and export a labeled MP4 (how the
        manuscript's supplementary videos were made). Built once, hidden;
        closing only hides it. The classifier picker and smoothing are the
        same variables as the Evaluate tab's."""
        win = tk.Toplevel(self.root)
        win.withdraw()
        win.title("Make Labeled Video")
        _sw, _sh = win.winfo_screenwidth(), win.winfo_screenheight()
        _w, _h = min(900, int(_sw * 0.7)), min(820, int(_sh * 0.85))
        ...
        ttk.Label(scrollable_frame, text="Make Labeled Video",
                  font=(FONT_FAMILY, 13, 'bold')).pack(anchor='w', padx=15, pady=(10, 0))
        ttk.Label(scrollable_frame,
                  text="Score one video with one classifier, check it with Preview with "
                       "Predictions, and export a labeled MP4 (optionally a clip). To score a "
                       "whole project use Run Classifiers; to check a classifier against human "
                       "labels use the Evaluate tab.",
                  foreground='gray', wraplength=700, justify='left').pack(anchor='w', padx=15, pady=(0, 4))

        # Classifier (same picker variable as the Evaluate tab)
        lv_clf = ttk.LabelFrame(scrollable_frame, text="Classifier", padding=10)
        lv_clf.pack(fill='x', padx=15, pady=8)
        ttk.Label(lv_clf, text="Classifier File:").grid(row=0, column=0, sticky='w', pady=2)
        self._lv_clf_combo = ttk.Combobox(
            lv_clf, textvariable=self.pred_classifier_path, width=46, state='readonly',
            values=list(self.pred_classifier_options.keys()))
        self._lv_clf_combo.grid(row=0, column=1, padx=5, pady=2)
        self._lv_clf_combo.bind('<<ComboboxSelected>>', self._on_pred_classifier_selected)
        ttk.Button(lv_clf, text="🔄", width=3,
                   command=self.refresh_pred_classifiers).grid(row=0, column=2, pady=2)
        ttk.Button(lv_clf, text="📁", width=3,
                   command=self.browse_pred_classifier).grid(row=0, column=3, pady=2)
        ttk.Button(lv_clf, text="📋 View Classifier Info",
                   command=self.view_pred_classifier_info).grid(row=1, column=1, sticky='w', pady=5)
        lv_smooth = ttk.Frame(lv_clf)
        lv_smooth.grid(row=2, column=0, columnspan=4, sticky='w', pady=(4, 2))
        ttk.Label(lv_smooth, text="Smoothing:").pack(side='left')
        for _lbl, _val in [("Bout filters", "bout_filters"),
                            ("HMM Viterbi", "hmm_viterbi"),
                            ("None", "none")]:
            ttk.Radiobutton(lv_smooth, text=_lbl, variable=self.pred_smoothing_mode,
                            value=_val).pack(side='left', padx=(8, 0))

        adv = scrollable_frame
        ...
        # Ethograms come from Tools -> Generate Ethogram; the old checkbox
        # here only logged "coming soon".
    ...
    def open_labeled_video_tool(self):
        """Show the Make Labeled Video window (rebuilt if it was destroyed)."""
        win = getattr(self, '_lv_window', None)
        try:
        ...
        if not alive:
            self._build_labeled_video_window()
            win = self._lv_window
        for _fn in (self.refresh_pred_classifiers, self.refresh_pred_videos):
            try:
                _fn()
            except Exception:
                pass
    ...
    # ================================================================== #
    # Evaluate tab, Compare section (human labels vs classifier)
    # ================================================================== #
    ...
    def _pr_run_compare(self):
        ...
        overrides = self._pr_read_overrides()
    ...
    def _pr_read_overrides(self):
        """The Evaluate tab's threshold / bout-filter overrides, read on the
        main thread: a dict (only the filled-in fields), None when the
        override is off or empty, or False after warning about bad input.
        Shared by Compare and Full evaluation."""
        if getattr(self, 'pred_override_params', None) is None or not self.pred_override_params.get():
            return None
        overrides = {}
        try:
    ...
    def _pr_set_overrides(self, thresh, min_bout, min_after, max_gap):
        """Fill the override fields (Optimize Parameters' result), turn the
        override on and open the row so the values are visible."""
        self.pred_thresh_override.set(f"{float(thresh):.3f}")
        self.pred_minbout_override.set(str(int(min_bout)))
        self.pred_minafter_override.set(str(int(min_after)))
        self.pred_maxgap_override.set(str(int(max_gap)))
        self.pred_override_params.set(True)
        if not getattr(self, '_pr_override_open', False) and hasattr(self, '_pr_toggle_override'):
            self._pr_toggle_override()
            ...
            for s in sessions:
                ...
                try:
                    ...
                    if obs.any():
                        ...
                        agree = 100.0 * acc
                        try:
                            # Unobserved frames -> 0 in both arrays (not dropped):
                            # dropping them splices neighbours and merges bouts.
                            bout_f1 = EvaluationTab._compute_bout_metrics(
                                _np.where(obs, _np.nan_to_num(yt), 0).astype(int),
                                _np.where(obs, yp, 0).astype(int),
                                fps=float(fps)).get('bout_f1')
                        except Exception:
                            bout_f1 = None
                    else:
    ...
    def _build_batch_run_panel(self, parent):
        ...
        try:
            ...
            pass
        _hero_row = ttk.Frame(hero_frame)
        _hero_row.pack(fill='x')
        hero_btn.pack(in_=_hero_row, side='left')
        _rv = ttk.Button(_hero_row, text="🏷 Review scoring…",
                         command=self.open_scoring_preview)
        _rv.pack(side='left', padx=(8, 0))
        try:
            from ui_tooltip import Tip as _Tip
            _Tip(_rv, "Play a scored video with the classifier calls overlaid.")
        except Exception:
            pass
        ttk.Label(hero_frame,
                  text="Scores every project video with the validated bundled set: licking, "
                       "scratching, facial grooming, body grooming, rearing, jumping, moving, "
        ...
        # List-editing buttons are disabled while a batch thread is alive
        # (the worker iterates a snapshot, but the UI should say so too).
        self._batch_list_btns = []
        _b = ttk.Button(pick_row, text="➕ Add",
                        command=self._batch_add_selected_classifier)
        _b.pack(side='left', padx=2)
        self._batch_list_btns.append(_b)
        _b = ttk.Button(pick_row, text="📁", width=3,
                        command=self.batch_add_classifier)
        _b.pack(side='left', padx=2)
        self._batch_list_btns.append(_b)
        ...
        _b = ttk.Button(btn_frame, text="➖ Remove",
                        command=self.batch_remove_classifier)
        _b.pack(side='left', padx=2)
        self._batch_list_btns.append(_b)
        ...
        _b = ttk.Button(btn_frame, text="🔄 Add All",
                        command=self.batch_autodetect_classifiers)
        _b.pack(side='left', padx=2)
        self._batch_list_btns.append(_b)
        _b = ttk.Button(btn_frame, text="📦 Add All Bundled",
                        command=self.batch_add_all_bundled)
        _b.pack(side='left', padx=2)
        self._batch_list_btns.append(_b)
        ...
        self.batch_apply_exclusions = tk.BooleanVar(value=True)
        _excl = ttk.Checkbutton(output_frame,
                        text="Resolve overlaps by priority: one behavior per frame "
                             "(licking > scratching > jumping > rearing > grooming > moving > still)",
                        variable=self.batch_apply_exclusions)
        _excl.grid(row=2, column=0, sticky='w', pady=2)
        try:
            from ui_tooltip import Tip as _Tip
            _Tip(_excl, "The bundled classifiers each score every frame on their own, so\n"
                        "two can fire on the same frame (a mouse walking while rearing,\n"
                        "grooming models firing on paw licking). With this on, each\n"
                        "behavior is zeroed where a higher-priority one fired, before bout\n"
                        "filtering and in the final calls, using the same order as the\n"
                        "Multi-Classifier and Sequencing tabs (body grooming ranks above\n"
                        "facial grooming). Off: every classifier's raw calls.")
        except Exception:
            pass
    ...
    def _ba_scan_sessions(self):
        ...
        # Same finder as the Analyze tabs: any CSV with Subject + Treatment
        # (key_file.csv first). Looking only for key_file.csv missed keys like
        # the example project's key.csv ("no key file", no groups shown).
        group_map = {}
        try:
            from project_config import find_key_files
            _keys = find_key_files(proj)
            if _keys:
                _kdf = pd.read_csv(_keys[0], encoding='utf-8-sig')
        ...
        for s in found:
            ...
            if not grp and group_map:   # token fallback: a key subject appearing in the name
                # whole underscore tokens, as everywhere else: a substring
                # test let 'mouse1' claim 'mouse10_veh'
                _nt = name.split('_')
                for subj, t in group_map.items():
                    _st = str(subj).split('_')
                    if subj and any(_nt[i:i + len(_st)] == _st
                                    for i in range(len(_nt) - len(_st) + 1)):
                        grp = t
                        break
    ...
    def _batch_add_classifier_path(self, path, label=None):
        ...
        self._batch_list_insert(path, label or os.path.basename(path))
        return True

    def _batch_list_insert(self, path, label):
        """Add a row to the classifier list and remember which file it is:
        bundled rows are labeled with the manifest name, not the file name."""
        self.batch_clf_listbox.insert(tk.END, label)
        if not hasattr(self, '_batch_label_paths'):
            self._batch_label_paths = {}
        self._batch_label_paths[label] = path

    def _batch_label_to_path(self, label):
        """The classifier path behind a list row, or None."""
        path = getattr(self, '_batch_label_paths', {}).get(label)
        if path in self.batch_classifiers:
            return path
        name = re.sub(r'^\[[^\]]+\]\s*', '', label)     # "[Project] x.pkl"
        for p in self.batch_classifiers:
    ...
    def create_tools_tab(self):
        ...
        def _add_section(parent, title, tools_list):
            ...
            for i, (text, cmd) in enumerate(tools_list):
                r, c = divmod(i, 2)
                btn = ttk.Button(gf, text=text, command=cmd, width=36)
        ...
        _add_section(tools_sf, "Video Tools", [
            ...
            ("🏷 Review Scoring (calls on video)", self.open_scoring_preview),
            ("🎬 Make Labeled Video", self.open_labeled_video_tool),
        ...
        _add_section(tools_sf, "Analysis Tools", [
            ...
            ("📊 Generate Ethogram", self.generate_ethogram),
        ...
        _add_section(tools_sf, "Configuration", [
            ...
            ("⚙️ Pre-extract Features", self.open_feature_extraction_run),
            ("📋 Key File (Group Assignment)", self._open_key_file_dialog),
            ("⏱ Diagnose Frame Rate", self.open_frame_rate_diagnose),
            ("⏱ Normalize Project Frame Rate", self.open_frame_rate_normalize),
        ])
    ...
    def open_skeleton_renderer(self):
        ...
        _render_script = os.path.join(
            _APP_DIR, 'render_skeleton_video.py'
        )
    ...
    def _create_active_learning_tab_v2(self):
        ...
        try:                                   # lab-only module, imported when the tab is built
            from active_learning_v2 import ActiveLearningTabV2
        except ImportError:
            ActiveLearningTabV2 = None
        if ActiveLearningTabV2 is not None:
            ...
            ttk.Label(err, text="Please ensure active_learning_v2.py is in the app folder next to PixelPaws_GUI.py",
    ...
    def _on_project_folder_changed(self, *_):
        ...
        # Videos dropped in the project root belong in videos/
        try:
            from project_config import gather_loose_media
            moved = gather_loose_media(folder)
        except Exception:
            moved = []
        if moved:
            self.root.after(500, lambda n=len(moved): self.set_status(
                f"Moved {n} file(s) from the project folder into videos/"))
        ...
        # Refresh the Evaluate tab's Compare session list for the new project.
        try:
            self._pr_scan_sessions()
        except Exception:
            pass
    ...
    def _load_key_file(self, folder: str):
        ...
        if not key_path:
            return
        try:
            # utf-8-sig: Excel's "CSV UTF-8" adds a BOM that otherwise turns the
            # header into '﻿Subject' and silently loads an empty key.
            n_blank = 0
            dupes = []
            ...
            return
        _kname = os.path.basename(key_path)
        if not self.key_file_data:
            print(f"Warning: key file {_kname} has no Subject rows "
                  f"(needs 'Subject' and 'Treatment' columns)")
        elif n_blank:
            print(f"Warning: key file {_kname}: {n_blank} row(s) with an empty "
                  f"Subject were ignored")
        if dupes:
            print(f"Warning: key file {_kname}: duplicate Subject row(s) "
                  f"{sorted(set(dupes))} - the last one wins")
    ...
    def _pick_roi_preview_session(self):
        ...
        if videos_dir and os.path.isdir(videos_dir):
            ...
            for v in vids:
                ...
                dlc_candidates = rank_h5(glob.glob(os.path.join(videos_dir, f"{base}DLC*.h5")))
                if dlc_candidates:
                    # Prefer filtered
    ...
    def _auto_convert_boris_in_behavior_labels(self, project_folder):
        ...
        for path in candidates:
            ...
            # A raw export already named <video>_labels.csv converts in place
            # (original archived first), so discovery still finds <video>_labels.csv.
            stem = os.path.splitext(os.path.basename(path))[0]
            if stem.lower().endswith('_labels'):
            ...
            try:
                ...
                # BORIS pairs START/STOP per (subject, behavior): two subjects can be
                # in the same behavior at once, and their bouts are OR-ed together.
                subject_col = _find_col(['Subject'])
                active_starts = {}   # (subject, behavior) -> START frame
                n_repeat_start = 0
                ...
                for _, row in df_sorted.iterrows():
                    ...
                    _subj = row[subject_col] if subject_col is not None else ''
                    key = ('' if pd.isna(_subj) else str(_subj), beh)
                    if beh_type == 'START':
                        if active_starts.get(key) is not None:
                            n_repeat_start += 1   # keep the earlier START
                        else:
                            active_starts[key] = frame_num
                    elif beh_type == 'STOP':
                        _st = active_starts.pop(key, None)
                        if _st is not None:
                            label_arrays[beh][max(0, _st):min(frame_num, n_frames)] = 1
                    elif beh_type == 'POINT':
                        if 0 <= frame_num < n_frames:
                            label_arrays[beh][frame_num] = 1

                for (_subj, _beh), _st in sorted(active_starts.items()):
                    self.log_train(
                        f"  ⚠️  {os.path.basename(path)}: unclosed START for "
                        f"'{_beh}'" + (f" (subject '{_subj}')" if _subj else "")
                        + f" at frame {_st} - dropped (no matching STOP)")
                if n_repeat_start:
                    self.log_train(
                        f"  ⚠️  {os.path.basename(path)}: {n_repeat_start} repeated "
                        f"START(s) before a STOP - kept the earlier START")

                # Archive first: out_path may be the original's own path.
    ...
    @staticmethod
    def _looks_like_boris_export(columns):
        """True for a raw BORIS event export (Behavior + Behavior type + Time
        columns) rather than a per-frame PixelPaws label CSV."""
        norm = {"".join(ch.lower() for ch in str(c) if ch.isalnum()) for c in columns}
        return (bool(norm & {'behavior', 'behaviour'})
                and bool(norm & {'behaviortype', 'type'})
                and bool(norm & {'time', 'times'}))

    def _maybe_convert_boris_exports(self, project_folder):
        """Convert raw BORIS exports in behavior_labels/ before session discovery.

        Discovery accepts behavior_labels/<video>.csv as a label file, so a raw BORIS
        export there was loaded as labels and the session skipped with "'X' column
        not in labels". Asks once per project, and only on the main thread (a worker
        thread never prompts and leaves the files alone). Declined exports are
        skipped at label-load time with a message saying why.
        """
        labels_dir = os.path.join(project_folder or '', 'behavior_labels')
        if not project_folder or not os.path.isdir(labels_dir):
            return
        boris_paths = []
        for _p in sorted(glob.glob(os.path.join(labels_dir, '*.csv'))
                         + glob.glob(os.path.join(labels_dir, '*.tsv'))):
            try:
                ...
                continue
            if self._looks_like_boris_export(_df.columns):
                boris_paths.append(_p)
        if not boris_paths:
            return

        decisions = getattr(self, '_boris_convert_decisions', None)
        if decisions is None:
            decisions = self._boris_convert_decisions = {}
        key = os.path.normcase(os.path.abspath(project_folder))
        ok = decisions.get(key)
        if ok is None and threading.current_thread() is threading.main_thread():
            ok = messagebox.askyesno(
                "Convert BORIS exports?",
                f"{len(boris_paths)} raw BORIS export(s) found in behavior_labels/:\n\n"
                + "\n".join(f"  • {os.path.basename(p)}" for p in boris_paths[:10])
                + ("\n  …" if len(boris_paths) > 10 else "")
                + "\n\nConvert them to per-frame label CSVs now? The originals are "
                  "moved to behavior_labels/boris_originals/.\n\n"
                  "If you decline, sessions whose labels file is a raw BORIS export "
                  "are skipped.")
            decisions[key] = bool(ok)
        if ok:
            try:
                _nc, _ns, _nf = self._auto_convert_boris_in_behavior_labels(project_folder)
                self.log_train(
                    f"BORIS conversion: {_nc} converted, {_ns} skipped, {_nf} failed "
                    f"(originals → behavior_labels/boris_originals/)")
            except Exception as _be:
                self.log_train(f"  ⚠️  BORIS conversion failed: {_be}")
        else:
            self.log_train(
                f"⚠️  {len(boris_paths)} raw BORIS export(s) in behavior_labels/ were "
                f"not converted; sessions using them will be skipped. Convert them "
                f"with Tools → BORIS to PixelPaws.")
    ...
    def find_training_sessions(self) -> List[Dict]:
        ...
        try:
            self._maybe_convert_boris_exports(project_folder)
        except Exception as _be:
            self.log_train(f"  ⚠️  BORIS export check failed: {_be}")
        ...
        for s in sessions:
            if s.get('target_path') and os.path.isfile(s['target_path']):
                try:
                    ...
                    if self._looks_like_boris_export(df_cols):
                        self.log_train(
                            f"⚠️  Skipping {os.path.basename(s['target_path'])}: raw BORIS "
                            f"export, not per-frame labels (Tools → BORIS to PixelPaws)")
                        continue
                    for col in df_cols:
            ...
            for s in sessions:
                if s.get('target_path') and os.path.isfile(s['target_path']):
                    try:
                        ...
                        # == 1, not sum() > 0: a -1 (unlabeled) tail outweighs the positives
                        if (_col_df[bname] == 1).any():
                            has_pos = True
                            break
                    except Exception:
                        pass
    ...
    @staticmethod
    def _early_stop_split(train_idx, session_ids, y, tail_frac=0.2):
        """Carve an early-stopping set out of a fold's TRAINING rows.

        Early stopping on the held-out fold tunes the tree count on the data being
        scored, which inflates OOF/CV F1. Instead hold out whole training sessions
        (inner GroupKFold); with a single training session, hold out the last
        ``tail_frac`` of its frames. Returns ``(fit_idx, es_idx, note)``;
        ``es_idx`` is empty when no usable set exists (fit without early stopping).
        """
        train_idx = np.sort(np.asarray(train_idx))
        groups = np.asarray(session_ids)[train_idx]
        n_sess = len(np.unique(groups))
        if n_sess >= 2:
            _inner = GroupKFold(n_splits=min(5, n_sess))
            _a, _b = next(iter(_inner.split(train_idx, groups=groups)))
            fit_idx, es_idx = train_idx[_a], train_idx[_b]
            note = (f"{len(np.unique(groups[_b]))} of {n_sess} training session(s) "
                    f"held out as the early-stopping set")
        else:
            n_es = int(round(len(train_idx) * tail_frac))
            if n_es < 1 or n_es >= len(train_idx):
                return train_idx, train_idx[:0], (
                    "training set too small for an early-stopping split - "
                    "early stopping off for this fold")
            fit_idx, es_idx = train_idx[:-n_es], train_idx[-n_es:]
            note = (f"only one training session - its last {n_es} frames "
                    f"(frame-wise tail split) are the early-stopping set")
        if not (np.asarray(y)[es_idx] == 1).any():
            return train_idx, train_idx[:0], (
                "early-stopping set has no positive frames - "
                "early stopping off for this fold")
        return fit_idx, es_idx, note
    ...
    def _run_cv_loop(self, X, y, session_ids, sessions, unique_sessions, kf,
        ...
        for fold, (train_idx, val_idx) in enumerate(_folds, 1):
            ...
            # Early-stopping set comes from the TRAINING sessions, never the
            # held-out fold (that would tune the tree count on the scored data).
            es_mask = None
            if use_early_stop:
                _fit_idx, _es_idx, _es_note = self._early_stop_split(
                    np.where(train_mask)[0], session_ids, y)
                if log:
                    self.log_train(f"  Early stopping: {_es_note}")
                if len(_es_idx):
                    train_mask = np.zeros(len(y), dtype=bool); train_mask[_fit_idx] = True
                    es_mask = np.zeros(len(y), dtype=bool); es_mask[_es_idx] = True
            ...
            if es_mask is not None:
                X_es = X[es_mask] if isinstance(X, np.ndarray) else X.loc[es_mask]
                fold_model.set_params(
                    early_stopping_rounds=early_stop_rounds)
                fold_model.fit(
                    X_train, y_train,
                    eval_set=[(X_es, y[es_mask])],
                    verbose=False)
            ...
            if log and self.train_viz_window:
                # Matplotlib drawing + winfo calls belong on the Tk main thread.
                def _add_fold(viz=self.train_viz_window, fold=fold, f1=f1,
                              prec=prec, rec=rec, elapsed=elapsed):
                    try:
                        if viz.window.winfo_exists():
                            viz.add_fold_result(fold, f1, prec, rec, elapsed)
                    except tk.TclError:
                        pass
                self._safe_after(_add_fold)
    ...
    def _run_feature_ladder(self, ranked, X, y, session_ids, sessions, unique_sessions,
        ...
        for N in rungs:
            ...
            if cv is None:
                break
            from cv_tuning import _sweep_postprocessing_fast as _al_sweep_fast
    ...
    def _optuna_tune(self, X, y, session_ids, use_spw, tree_method, early_stop_rounds=30):
        ...
        Returns ``(None, None)`` when there are fewer than 2 sessions: session-level
        CV is impossible (GroupKFold needs n_splits >= 2), so tuning is skipped."""
        import optuna
        ...
        kf = GroupKFold(n_splits=n_folds)   # frame-balanced, congruent with the main CV
        ...
        def objective(trial):
            ...
            for train_idx, val_idx in kf.split(session_ids, y, groups=session_ids):
                # Early-stopping set from the training sessions, not the scored fold.
                fit_idx, es_idx, _ = self._early_stop_split(train_idx, session_ids, y)
                train_mask = np.zeros(len(y), dtype=bool); train_mask[fit_idx] = True
                val_mask = np.zeros(len(y), dtype=bool); val_mask[val_idx] = True
                ...
                if len(es_idx):
                    es_mask = np.zeros(len(y), dtype=bool); es_mask[es_idx] = True
                    m.set_params(early_stopping_rounds=early_stop_rounds)
                    m.fit(X_tr, y_tr, eval_set=[(X[es_mask], y[es_mask])], verbose=False)
                else:
        ...
        try:
            ...
            for i, session in enumerate(sessions):
                ...
                try:
                    ...
                    _why = str(_ke.args[0]) if _ke.args else ''
                    if 'BORIS' not in _why:
                        _why = f"'{behavior_name}' column not in labels"
                    self.log_train(
                        f"  Session {i+1}/{len(sessions)}: skipped ({_why})")
                    skipped += 1
                    continue
                all_X.append(X_s)
                all_y.append(y_s)
            ...
            if self.train_use_optuna.get():
                ...
                try:
                    ...
                    if optuna_best_hp is None:
                        self.log_train(
                            "  Optuna skipped: tuning is cross-validated across "
                            "sessions and needs at least 2 labeled sessions. "
                            "Using the manual hyperparameters.")
                    else:
                        self.log_train(f"\n  Best hyperparameters (AP={optuna_best_ap:.4f}):")
                        for k, v in optuna_best_hp.items():
                            self.log_train(f"    {k}: {v}")
                        # Apply to UI vars so CV + final model use them. Tk variables
                        # are set on the main thread; wait so CV reads the new values.
                        _hp_applied = threading.Event()

                        def _apply_hp(hp=dict(optuna_best_hp)):
                            try:
                                self.train_max_depth.set(hp['max_depth'])
                                self.train_learning_rate.set(hp['learning_rate'])
                                self.train_colsample.set(hp['colsample_bytree'])
                                self.train_subsample.set(hp['subsample'])
                                self.train_n_estimators.set(hp['n_estimators'])
                            finally:
                                _hp_applied.set()
                        ...
                        if threading.current_thread() is threading.main_thread():
                            ...
                            self._safe_after(_apply_hp)
                            if not _hp_applied.wait(timeout=30):
                                self.log_train(
                                    "  ⚠️  Tuned hyperparameters were not applied in "
                                    "time; this run may use the manual values.")
                except ImportError:
            ...
            # Session-level CV needs >= 2 sessions (GroupKFold(n_splits=1) raises).
            # With one, skip CV and every OOF step and go straight to the final fit.
            skip_cv = len(unique_sessions) < 2
            ...
            if _elig_mode != 'off' and not skip_cv:
                from cv_tuning import (session_positive_counts as _spc,
            ...
            # GroupKFold over FRAMES (groups=session_ids). Folds are
            # frame-balanced (uniform train-set sizes), the field-standard for grouped data.
            if skip_cv:
                kf = None
                self.log_train(
                    "  ⚠️  CV SKIPPED - only 1 labeled session. Session-level CV needs "
                    "at least 2, so no CV / OOF / honest F1 is computed. The final "
                    "model is fit on all labeled frames with the manual threshold "
                    "(0.50) and bout settings. Label a second session for an honest "
                    "estimate and tuned post-processing.")
            else:
            ...
            if skip_cv:
                # Nothing is held out, so there are no OOF probabilities.
                cv_result = {'oof_proba': np.full(len(y), np.nan), 'fold_of': None,
                             'fold_f1_scores': [], 'fold_precisions': [],
                             'fold_recalls': [], 'fold_best_iters': [],
                             'fold_models': [], 'fold_val_masks': []}
            else:
            ...
            if self.train_learning_curve.get() and not skip_cv:
                self.log_train("\n" + "=" * 60)
            ...
            if not skip_cv and not np.all(oof_valid_mask):
            ...
            if skip_cv:
                if self.train_use_calibration.get():
                    self.log_train(
                        "\n  ⚠️  Calibration skipped - it is fit on OOF predictions "
                        "and CV was skipped.")
            elif self.train_use_calibration.get() and len(np.unique(y)) < 2:
                self.log_train(
            ...
            # - chooses the deployed threshold/bout params. Uses cv_tuning's parallel,
            # session-aware sweep; identical grid to the serial _sweep_postprocessing.
            from cv_tuning import (_sweep_postprocessing_fast as _al_sweep_fast,
                                   _honest_pipeline_oof_f1 as _al_honest_pipeline)
            if skip_cv:
                # No OOF to tune on: deploy the manual settings, untuned.
                best_params = {'thresh': 0.5,
                               'min_bout': int(self.train_min_bout.get()),
                               'min_after_bout': int(self.train_min_after_bout.get()),
                               'max_gap': int(self.train_max_gap.get()),
                               'f1': float('nan')}
                self.log_train("\n  Post-processing (manual, not tuned - CV skipped):")
            else:
            ...
            try:
                ...
                if not skip_cv and 0 < int(np.sum(y)) < len(y):
            ...
            if fold_of is not None and honest_f1 is not None:
                try:
                    from cv_tuning import _honest_hmm_oof_f1 as _al_hmm
            ...
            try:
                # (import first: it binds _apply_bout_filtering for the rest of
                # this method)
                from evaluation_tab import _apply_bout_filtering, EvaluationTab
                if skip_cv:
                    raise RuntimeError("no OOF predictions - CV skipped")
            ...
            if _ladder_mode and skip_cv:
                self.log_train("\n  ⚠️  SHAP ladder skipped - it compares CV F1 across "
                               "feature counts and CV was skipped (1 labeled session).")
                _ladder_mode = False
            if _ladder_mode:
                try:
            ...
            run_lovo = not skip_cv and (
                (opt_strategy == 'lovo') or (opt_strategy == 'auto' and len(included_sessions) >= 2))
            ...
            if run_lovo:
                ...
                if skip_cv:
                    self.log_train("  → CV skipped - no LOVO sweep; manual params saved")
                elif opt_strategy == 'oof':
            ...
            # Full performance plots - optional (they plot OOF predictions)
            if self.train_generate_plots.get() and skip_cv:
                self.log_train("\n  (Performance plots skipped - they need OOF "
                               "predictions and CV was skipped)")
            elif self.train_generate_plots.get():
            ...
            if skip_cv:
                self.log_train(
                    f"CV skipped (1 labeled session) - no performance estimate. "
                    f"Saved manual params: thresh={best_params['thresh']:.2f}, "
                    f"min_bout={best_params['min_bout']}, "
                    f"min_after={best_params['min_after_bout']}, "
                    f"max_gap={best_params['max_gap']}")
            else:
            ...
            # Small-dataset next-step recommendation, for when label quantity/quality
            ...
            try:
                ...
                if (len(included_sessions) < 10 and
                    ...
                    self.log_train(
                        "not model tweaks. Look at the frames where this classifier and your")
                    self.log_train(
                        "labels disagree (Evaluate tab) - typically")
                    self.log_train(
            ...
            # Store results (the Evaluate and Run tabs pre-fill from the last trained classifier)
    ...
    def extract_features_for_session(self, session, cfg, cache_root, behavior_name):
        ...
        if os.path.isfile(feature_cache_file):
            ...
            # handles both joblib+LZ4 and plain pickle.
        ...
        if behavior_name not in y_df.columns:
            if self._looks_like_boris_export(y_df.columns):
                raise KeyError(
                    f"{os.path.basename(session['target_path'])} is a raw BORIS export, "
                    f"not per-frame labels - convert it with Tools → BORIS to PixelPaws")
        ...
        if _min_lb > 0 or _max_lb > 0:
            from cv_tuning import exclude_bouts_by_length
    ...
    def _collect_provenance_meta(self) -> dict:
        ...
        try:
            ...
            sha = 'unknown'
        try:
            from pp_version import __version__ as app_version
        except Exception:
            app_version = 'unknown'
        meta = {
            'pixelpaws_version': app_version,
            'code_version':    sha,
    ...
    def _generate_learning_curve_plot(self, lc_results, n_pos_frames,
        ...
        Single red line + markers. `lc_results` is the list of ``(fraction, F1)`` pairs
    ...
    # ============================ Problem frames (pose refinement) ============================
    ...
    def _ensure_default_bundle_seeded(self, show_error=True):
        """Copy the shipped default model bundle into the per-user bundles dir (first run, or
        when the shipped one is newer) and mark it active if nothing is active yet. Returns
        True if the DLC inference engine imported cleanly, else False (optionally showing an
        error dialog)."""
    ...
    def add_videos_to_project(self, on_done=None):
        ...
        def _worker(do_transcode):
            ...
            if do_transcode:
                try:
                    from pp_runtime import build_transcode_cmd, transcode_output_ok
            ...
            def _copy_in(srcp):
                """Copy a source into videos/ without re-encoding. One-click
                scoring only picks up .mp4/.avi, so a .mov/.mkv (typically an
                already-H.265 camera file) is remuxed into an .mp4 container
                (stream copy, calibration tags kept) when ffmpeg is present;
                plain copy otherwise. Returns the destination, or None when
                the import was cancelled mid-remux."""
                if ffmpeg_ok and srcp.suffix.lower() in ('.mov', '.mkv'):
                    dst = Path(videos_dir) / f"{srcp.stem}.mp4"
                    _log(f"⟳ {srcp.name}: remuxing to .mp4 (stream copy, no re-encode)...")
                    try:
                        proc = subprocess.Popen(
                            ["ffmpeg", "-y", "-v", "error", "-i", str(srcp),
                             "-c", "copy", "-map_metadata", "0",
                             "-movflags", "use_metadata_tags", str(dst)],
                            stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL,
                            stderr=subprocess.DEVNULL)
                        proc_box[0] = proc
                        proc.wait()
                        proc_box[0] = None
                        if (not cancel_evt.is_set() and proc.returncode == 0
                    ...
                    if cancel_evt.is_set():
                        return None
                    _log(f"✗ {srcp.name}: remux failed - copying as-is")
            ...
            for i, src in enumerate(paths, 1):
                ...
                try:
                    ...
                    if _tc:
                        try:
                            from pp_runtime import is_hevc
                            if is_hevc(srcp):
                                _log(f"↷ {srcp.name}: already H.265, copying as-is "
                                     "(no re-encode)")
                                _tc = False
                        except Exception:
                            pass
    ...
    def _auto_run_gait(self):
        ...
        if tab is None or not hasattr(tab, 'auto_run'):
            return
        try:
            self.notebook.select("🐾 Paw Contour")
        except Exception:
            pass
        try:
            tab.auto_run()
    ...
    def _pf_best_snapshot(self, project_dir):
        ...
        cands = _glob.glob(os.path.join(project_dir, 'dlc-models-pytorch', 'iteration-*',
                                        '*trainset*shuffle*', 'train', 'snapshot-*.pt'))
        def key(p):
    ...
    def _pf_retrain_dialog(self, proj, meta, sessions):
        ...
        def _worker():
            ...
            try:
                try:                       # lab machines keep a separate DeepLabCut environment
                    from pp_config import DLC_PYTHON
                except Exception:
                    from pp_runtime import resolve_pose_python
                    DLC_PYTHON = resolve_pose_python()
            except Exception:
    ...
    def open_scoring_preview(self):
        """Play a scored project video with every classifier's calls overlaid
        (reads results/per_frame sheets, falls back to *_predictions.csv)."""
        from scoring_preview import open_scoring_preview
        open_scoring_preview(self.root, self.current_project_folder.get())
    ...
    def _run_brightness_diagnostics_thread(self, file_path, progress_window, progress_text):
        ...
        Never switches the matplotlib backend (the app stays on Agg) and never
        touches pyplot here: the plot functions run on the main thread below."""
        try:
            import seaborn as sns
            ...
            # brightness_diagnostics draws with pyplot, whose global figure state
            # is not thread-safe, so the plotting runs on the main thread (Agg,
            # files only) and then opens the results window.
            def _plot_and_show():
                try:
                    if os.path.isfile(script_path):
                        ...
                        spec = importlib.util.spec_from_file_location(
                            "brightness_diagnostics", script_path)
                        bd_module = importlib.util.module_from_spec(spec)
                        spec.loader.exec_module(bd_module)

                        # The table plot needs the module's own statistics
                        # (NaN-aware, with Pct_NaNs); the local stats_df above
                        # lacks that column and raised KeyError here.
                        nonlocal stats_df
                        stats_df = bd_module.calculate_statistics(
                            features_df, brightness_cols)
                ...
                self._show_brightness_results_window(output_dir, stats_df, bodyparts)

            self._safe_after(_plot_and_show)
    ...
    def show_brightness_preview(self):
        ...
        script_path = os.path.join(_APP_DIR, 'brightness_preview.py')
    ...
    def correct_crop_offset_single(self):
        ...
        script_path = os.path.join(_APP_DIR, 'correct_features_crop.py')
    ...
    def crop_video_for_dlc(self):
        ...
        script_path = os.path.join(_APP_DIR, 'crop_for_dlc.py')
        if not os.path.isfile(script_path):
            messagebox.showerror("Not Found",
...
"""
    ...
    def generate_ethogram(self):
        """Ethogram report (time budget, bout distributions, raster,
        summary) for every session found in a predictions folder."""
        proj = self.current_project_folder.get()
        start = os.path.join(proj, 'results') if proj else ''
        folder = filedialog.askdirectory(
            title="Folder with *_predictions.csv (e.g. the project's results/)",
            initialdir=start if os.path.isdir(start) else None)
        if not folder:
            return
        import glob as _g
        paths = _g.glob(os.path.join(folder, "**", "*_predictions.csv"),
                        recursive=True)
        if not paths:
            messagebox.showinfo("No predictions",
                                "No *_predictions.csv found in that folder. "
                                "Run classifiers first.")
            return
        # Group files by session stem (strip the classifier token).
        sessions = {}
        for p in paths:
            stem = os.path.basename(p)[:-len("_predictions.csv")]
            beh = None
            try:
                head = pd.read_csv(p, nrows=0)
                beh = next((c for c in head.columns if c.lower() not in
                            ("frame", "probability", "prob")), None)
            except Exception:
                continue
            if beh is None:
                continue
            for tok in ("_classifier", "_PixelPaws"):
                if tok in stem:
                    ...
                    break
            sessions.setdefault(stem, {})[beh] = p
        if not sessions:
            messagebox.showinfo("No predictions",
                                "Could not read any prediction CSVs there.")
            return
        out_root = os.path.join(proj or folder, 'analysis', 'ethograms')
        n_done = 0
        try:
            for sess, behs in sorted(sessions.items()):
                preds = {}
                for beh, p in behs.items():
                    try:
                        d = pd.read_csv(p)
                        col = next(c for c in d.columns if c.lower() not in
                                   ("frame", "probability", "prob"))
                        preds[beh] = d[col].values.astype(int)
                    except Exception:
                        continue
                if not preds:
                    continue
                ...
                if proj:
                    for ext in ('.mp4', '.avi', '.mov'):
                        vp = os.path.join(proj, 'videos', sess + ext)
                        if os.path.isfile(vp):
                            try:
                                import cv2 as _cv2
                                cap = _cv2.VideoCapture(vp)
                                f = cap.get(_cv2.CAP_PROP_FPS)
                                cap.release()
                                if f and f > 0:
                                    fps = float(f)
                            except Exception:
                                pass
                            break
                EthogramGenerator.generate_ethogram(
                    preds, fps, os.path.join(out_root, sess))
                n_done += 1
        except Exception as e:
            messagebox.showerror("Ethogram", f"Failed: {e}")
            return
        if n_done:
            messagebox.showinfo(
                "Ethogram",
                f"Generated ethograms for {n_done} session(s):\n{out_root}")
            try:
                os.startfile(out_root)
            except Exception:
                pass
        else:
            messagebox.showinfo("Ethogram", "No usable predictions found.")
    ...
    def toggle_theme(self):
        ...
        # apply_theme first: ttkbootstrap's style republish would otherwise wipe
        # the sidebar colors that update_theme sets.
        self.apply_theme()
    ...
    def convert_boris_to_pixelpaws(self):
        ...
        def do_convert_one(boris_path, behavior_name, fps_text, output_dir, all_behaviors=False):
            ...
            # Build per-behavior label arrays. BORIS pairs START/STOP per
            # (subject, behavior), so two subjects' overlapping bouts are OR-ed.
            label_arrays = {str(b): np.zeros(n_frames, dtype=int) for b in behaviors_to_extract}
            ...
            for (_subj, _beh), _st in sorted(active_starts.items()):
                log_status(f"  ⚠ unclosed START for '{_beh}'"
                           + (f" (subject '{_subj}')" if _subj else "")
                           + f" at frame {_st} - dropped (no matching STOP)", 'orange')
            if n_repeat_start:
                log_status(f"  ⚠ {n_repeat_start} repeated START(s) before a STOP "
                           f"- kept the earlier START", 'orange')
    ...
    def show_docs(self):
        """Open the bundled PixelPaws guide (docs/pixelpaws.md; the README
        is only the overview, install and Quick Start). Falls back to the
        README, then to the GitHub page."""
        import webbrowser
        here = os.path.dirname(os.path.abspath(__file__))
        for local in (os.path.join(here, "docs", "pixelpaws.md"),
            ...
            if os.path.isfile(local):
                try:
                    ...
                    pass
        webbrowser.open("https://github.com/rslivicki/PixelPaws/blob/main/docs/pixelpaws.md")
...
Left/Right       - Previous/Next frame (in video preview)
"""
    ...
    def bundled_priority_order(self):
        """The manifest's 'priority_order' (highest first): the order the
        Multi-Classifier and Sequencing tabs resolve states in. [] if absent."""
        import json as _json
        man = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           'pixelpaws_global_classifier_encyclopedia', 'manifest.json')
        try:
            with open(man, encoding='utf-8') as fh:
                return [str(b) for b in (_json.load(fh).get('priority_order') or [])]
        except Exception:
            return []

    def bundled_suppression_rules(self):
        """{behavior: [overriding behaviors]} for the bundled classifiers.

        From the manifest's 'priority_order' (every behavior is overridden by
        each one ranked above it, so a frame keeps only its highest-priority
        call) plus any per-classifier 'suppressed_by' entries (licking over
        facial grooming since v1.1.2, over body grooming since v1.1.3)."""
        import json as _json
        man = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           'pixelpaws_global_classifier_encyclopedia', 'manifest.json')
        rules = {}
        prio = self.bundled_priority_order()
        for i, b in enumerate(prio):
            if i:
                rules[b] = list(prio[:i])
        try:
            with open(man, encoding='utf-8') as fh:
                for c in _json.load(fh).get('classifiers', []):
                    src = c.get('suppressed_by') or []
                    if src and c.get('name'):
                        cur = rules.setdefault(c['name'], [])
                        cur.extend(s for s in src if s not in cur)
        except Exception:
            pass
        return rules
    ...
    def refresh_pred_classifiers(self):
        ...
        for _cb in (getattr(self, 'pred_classifier_combo', None),
                    getattr(self, '_lv_clf_combo', None)):
            try:
                if _cb is not None:
                    _cb['values'] = list(self.pred_classifier_options.keys())
            except tk.TclError:
                pass
    ...
    def refresh_pred_videos(self):
        ...
        if hasattr(self, 'pred_video_combo'):
            try:     # lives in the Make Labeled Video window
    ...
    def _auto_find_pred_files(self):
        ...
        if not self.pred_dlc_path.get():
            from h5_resolve import rank_h5
            dlc_files = rank_h5(glob.glob(os.path.join(video_folder, f"{video_base}DLC*.h5")))
            if dlc_files:
    ...
    def auto_find_dlc(self):
        ...
        # Look for matching DLC file (active network's first)
        from h5_resolve import rank_h5
        dlc_files = rank_h5(glob.glob(os.path.join(video_folder, f"{video_base}DLC*.h5")))
    ...
    def preview_with_predictions(self):
        ...
        try:
            ...
            # This runs on a worker thread: every widget touch goes through
            ...
            def run_prediction():
                try:
                    # Load classifier
                    _set_status("Loading classifier...")
                    ...
                    if features_path and os.path.isfile(features_path):
                        _set_status("Loading pre-extracted features...")
                    ...
                    if not features_loaded:
                        _set_status("Loading / extracting features...")
                        if not cache_file:
                    ...
                    # Predict
                    _set_status("Running classifier...")
                    ...
                    _set_status("Applying bout filtering...")
                    ...
                    _set_status("Checking for human labels...")
                    ...
                    def open_preview_on_main_thread():
                        ...
                        SideBySidePreview(
                            ...
                            human_labels=result_data['human_labels'],
                            overlay_colors={
                    ...
                    self._safe_after(open_preview_on_main_thread)
    ...
    def _predict_thread(self):
        ...
        try:
            ...
            # "Create labeled video" = the Export Labeled Video button, run
            # automatically once the predictions exist.
            if self.pred_save_video is not None and self.pred_save_video.get():
                self._pred_log("Creating the labeled video...\n")
                self.root.after(0, self.export_labeled_video)   # main thread
    ...
    def batch_add_classifier(self):
        ...
        if filepath and filepath not in self.batch_classifiers:
            ...
            self._batch_list_insert(filepath, os.path.basename(filepath))
    ...
    def batch_remove_classifier(self):
        ...
        if selection:
            ...
            path = self._batch_label_to_path(item)
            if path is not None:
    ...
    def batch_edit_classifier(self):
        ...
        clf_path = self._batch_label_to_path(item)
        if not clf_path:
            messagebox.showwarning("Edit Settings",
                                   f"Could not find the classifier file for {item}.")
            return
            ...
            for f in sorted(os.listdir(clf_dir)):
                if f.endswith('.pkl'):
                    ...
                    if path not in self.batch_classifiers:
                        ...
                        self._batch_list_insert(path, f"[Project] {f}")
                        added += 1
        ...
        if os.path.isdir(gcf):
            for f in sorted(os.listdir(gcf)):
                if f.endswith('.pkl'):
                    ...
                    if path not in self.batch_classifiers:
                        self.batch_classifiers[path] = {'min_bout_sec': 0.2, 'bin_size_sec': 60}
                        self._batch_list_insert(path, f"[Global] {f}")
                        added += 1
    ...
    def find_dlc_for_video(self, video_path: str, folder: str, prefer_filtered: bool = True):
        ...
        video_base = os.path.splitext(os.path.basename(video_path))[0]
        # DLC names its output <stem>DLC<scorer>...; anchoring on the "DLC"
        # keeps M1.mp4 from picking up M1_croppedDLC*.h5 (a "(?!\d)" guard
        # only excluded M10).
        pattern = re.compile(rf"^{re.escape(video_base)}DLC", re.IGNORECASE)
        ...
        # Active network's file first when a video was re-tracked (h5_resolve).
        from h5_resolve import rank_h5
        filtered, unfiltered = rank_h5(filtered), rank_h5(unfiltered)
    ...
    def run_default_classifier_set(self, select_tab=True, only_videos=None):
        ...
        untouched. Returns True only when the batch thread actually started
        (the Quick Start tab fails its Classifiers stage otherwise)."""
        ...
        return bool(self._start_batch_run())
    ...
    def _start_batch_run(self):
        ...
        self._set_batch_list_buttons('disabled')
        self._batch_thread = threading.Thread(target=self._batch_analysis_thread,
                                              daemon=True)
        self._batch_thread.start()
        return True

    def _set_batch_list_buttons(self, state):
        """Enable/disable the Add / Remove / Add-All buttons of the batch
        classifier list (locked while a batch thread is alive)."""
        for _b in getattr(self, '_batch_list_btns', ()):
            try:
                _b.config(state=state)
            except tk.TclError:
                pass
    ...
    def _batch_analysis_thread(self):
        ...
        try:
            ...
            # Snapshot the list: the dict must not be iterated live while
            # the Add / Remove buttons could still mutate it.
            _batch_items = list(self.batch_classifiers.items())
            if not _batch_items:
            ...
            self._batch_log(f"Using {len(_batch_items)} classifier(s)\n\n")
            ...
            # Exclusion rules (manifest 'priority_order' + 'suppressed_by'): a
            # behavior is zeroed where a higher-priority one fired, so the
            # higher one must be final first. Within a richness group the
            # classifiers run in priority order; anything scored out of order
            # is re-derived in the priority pass after each video.
            _excl_rules = self.bundled_suppression_rules()
            _prio_names = self.bundled_priority_order()
            for _b, _srcs in _excl_rules.items():
                for _s in _srcs + [_b]:
                    if _s not in _prio_names:
                        _prio_names.append(_s)

            def _prio_rank(_cp):
                b = os.path.basename(str(_cp)).lower()
                for _i, _nm in enumerate(_prio_names):
                    if _nm.lower() in b:
                        return _i
                return len(_prio_names)
            _ordered = sorted(_batch_items,
                              key=lambda kv: _richness(kv[0]) + (-_prio_rank(kv[0]),),
                              reverse=True)
            # behaviors of the rule set that this batch actually scores:
            # a rule only applies between classifiers that are both in it
            _in_batch = {_nm for _nm in _prio_names
                         if any(_nm.lower() in os.path.basename(str(_cp)).lower()
                                for _cp, _ in _ordered)}
            _prio_idx = {_nm: _i for _i, _nm in enumerate(_prio_names)}
            _batch_items = _ordered
            self.batch_classifiers = dict(_ordered)
            _apply_excl = bool(getattr(self, 'batch_apply_exclusions', None) is None
                               or self.batch_apply_exclusions.get())
            # The rules come from the encyclopedia manifest: predictions
            # written before a rule existed are stale once it appears.
            _excl_manifest = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                          'pixelpaws_global_classifier_encyclopedia', 'manifest.json')
            ...
            def _write_clf_outputs(video_base, clf_base, behavior_name, y_proba,
                                   y_pred_filtered, fps, results_folder,
                                   bin_size=60.0):
                """Predictions CSV, bouts CSV and time-binned CSV for one
                video x classifier; returns the stats the summary row needs.
                Used by the main loop and again by the exclusion post-pass,
                so a re-derived behavior writes exactly the same files."""
                ...
                n_bouts, bouts = bout_stats['n_bouts'], bout_stats['bouts']
                ...
                if save_labels:
                    ...
                    pd.DataFrame({'frame': np.arange(n_frames), 'probability': y_proba,
                                  behavior_name: y_pred_filtered}).to_csv(output_csv, index=False)
                    self._batch_log(f"     ✓ Saved predictions CSV\n")
                    bouts_csv = os.path.join(results_folder, f"{video_base}_{clf_base}_bouts.csv")
                    if bouts:
                        pd.DataFrame([{'start_frame': b['start'], 'end_frame': b['end'],
                                       'duration_frames': b['duration_frames'],
                                       'duration_sec': b['duration_sec']} for b in bouts]
                                     ).to_csv(bouts_csv, index=False)
                        self._batch_log(f"     ✓ Saved bouts CSV ({n_bouts} bouts)\n")
                    elif os.path.isfile(bouts_csv):
                        os.remove(bouts_csv)      # re-derived to zero bouts
                frames_per_bin = max(int(bin_size * fps), 1)
                ...
                for bin_idx in range(n_bins):
                    ...
                    timebin_data.append({
                        'bin': bin_idx, 'start_frame': start_frame, 'end_frame': end_frame,
                ...
                pd.DataFrame(timebin_data).to_csv(timebin_csv, index=False)
                self._batch_log(f"     ✓ Saved time-binned summary\n")
                return {'n_frames': n_frames, 'n_bouts': n_bouts, 'percent_behavior': percent_behavior,
                        'mean_bout_duration': mean_bout_duration, 'timebin_csv': timebin_csv}

            total_operations = len(videos) * len(_batch_items)
            current_operation = 0
            ...
            for video_path in videos:
                ...
                if not dlc_path:
                    ...
                    current_operation += len(_batch_items)
                ...
                for clf_path, settings in _batch_items:
                    ...
                    try:
                        ...
                        if use_override:
                            ...
                            # apply_smoothing (and the exclusion post-pass, which
                            # reuses this clf_data) read the pkl's own values, so
                            # the overrides must live on the dict, as in
                            # _pr_compare_worker.
                            clf_data = dict(clf_data)
                            clf_data.update({'best_thresh': best_thresh,
                                             'min_bout': min_bout,
                                             'min_after_bout': min_after,
                                             'max_gap': max_gap})
                        ...
                        # A behavior whose higher-priority classifiers in this
                        # batch are not scored yet (they sit in a later feature-
                        # cache group) is scored fresh, so the priority pass can
                        # resolve it against their final calls.
                        _pending_src = (_apply_excl and any(
                            _s in _in_batch and f"{_s}_pred" not in _pf_cols
                            for _s in _excl_rules.get(behavior_name, [])))
                        if not use_override and not _pending_src and os.path.isfile(_skip_csv):
                            # A behavior under an exclusion rule also depends on
                            # the overriding classifiers (model + predictions)
                            # and on the rule itself being on when it was scored.
                            _src_preds = []
                            if behavior_name in _excl_rules:
                                for _src in _excl_rules[behavior_name]:
                                    for _sp, _ in _batch_items:
                                        if _src.lower() not in os.path.basename(str(_sp)).lower():
                                            continue
                                        _sp_csv = os.path.join(
                                            os.path.dirname(results_folder), _src,
                                            f"{video_base}_{os.path.splitext(os.path.basename(_sp))[0]}_predictions.csv")
                                        if os.path.isfile(_sp_csv):
                                            _src_preds.append((_src, _sp, _sp_csv))
                            try:
                                ...
                                if _apply_excl and _src_preds:
                                    if os.path.isfile(_excl_manifest):
                                        _deps.append(os.path.getmtime(_excl_manifest))
                                    for _src, _sp, _sp_csv in _src_preds:
                                        _deps += [os.path.getmtime(_sp), os.path.getmtime(_sp_csv)]
                                _fresh = os.path.getmtime(_skip_csv) >= max(_deps)
                            except OSError:
                                _fresh = False
                            if _fresh:
                                try:
                                    # round_trip: the default parser is off by one unit in
                                    # the last place for many values, so reused
                                    # probabilities would not equal the fresh ones
                                    _prev = pd.read_csv(_skip_csv, float_precision="round_trip")
                                    _prev_pred = (_prev[behavior_name].values
                                                  if behavior_name in _prev.columns
                                                  else _prev.iloc[:, -1].values)
                                    # Scored with the rule in the other state?
                                    # With the rule on, probability is exactly 0
                                    # wherever the overriding behavior fired.
                                    for _src, _sp, _sp_csv in _src_preds:
                                        _m = _pf_cols.get(f"{_src}_pred")
                                        if _m is None:
                                            _m = pd.read_csv(_sp_csv).iloc[:, -1].values
                                        _n = min(len(_m), len(_prev))
                                        _hit = np.asarray(_m[:_n]) == 1
                                        if not _hit.any():
                                            continue
                                        _zeroed = not (np.asarray(_prev['probability'].values[:_n],
                                                                  dtype=float)[_hit] != 0).any()
                                        if _zeroed != _apply_excl:
                                            raise ValueError(
                                                f"scored with the {_src} exclusion rule "
                                                f"{'off' if _apply_excl else 'on'}")
                        ...
                        # Exclusion rules: this behavior is zeroed on frames
                        # where an overriding behavior (already scored for this
                        # video) fired, before bout filtering and again in the
                        # final calls (gap bridging would refill them).
                        _excl_mask = None
                        _raw_proba = np.asarray(y_proba, dtype=float).copy()
                        if _apply_excl and behavior_name in _excl_rules:
                            _excl_mask = np.zeros(len(y_proba), dtype=bool)
                            _hits = []
                            for _src in _excl_rules[behavior_name]:
                                if _src not in _in_batch:
                                    continue
                                _m = _pf_cols.get(f"{_src}_pred")
                                if _m is None:
                                    # scored later in this batch: the priority
                                    # pass after this video re-derives it
                                    continue
                                _n = min(len(_m), len(y_proba))
                                _hit = np.asarray(_m[:_n]) == 1
                                _excl_mask[:_n] |= _hit
                                if _hit.any():
                                    _hits.append(f"{_src} {int(_hit.sum())}")
                            if _hits:
                                self._batch_log(f"     Priority: {behavior_name} zeroed on "
                                                f"{int(_excl_mask.sum())} frames where a higher-"
                                                f"priority behavior fired ({', '.join(_hits)})\n")

                        # Apply smoothing (respects the predict-tab smoothing-mode choice)
                        _smooth = (self.pred_smoothing_mode.get()
                                   if self.pred_smoothing_mode is not None
                                   else 'bout_filters')
                        y_proba, y_pred_filtered = smooth_with_exclusions(
                            y_proba, clf_data, _smooth, _excl_mask)
                        ...
                        # time-bin width from ⚙ Edit Settings (60 s default)
                        try:
                            _bin_s = float(settings.get('bin_size_sec') or 60.0)
                        except (TypeError, ValueError):
                            _bin_s = 60.0
                        _bin_s = _bin_s if _bin_s > 0 else 60.0
                        _out = _write_clf_outputs(video_base, clf_base, behavior_name,
                                                  y_proba, y_pred_filtered, fps, results_folder,
                                                  bin_size=_bin_s)
                        n_frames, n_bouts = _out['n_frames'], _out['n_bouts']
                        percent_behavior = _out['percent_behavior']
                        mean_bout_duration = _out['mean_bout_duration']
                        _pf_cols[f"{_pf_key}_prob"] = np.asarray(y_proba)
                        _pf_cols[f"{_pf_key}_pred"] = np.asarray(y_pred_filtered)
                        _pf_n = max(_pf_n, n_frames)
                        batch_timebins_files.append((behavior_name, clf_base, video_path, _out['timebin_csv']))
                        ...
                        if _apply_excl and behavior_name in _prio_idx:
                            _deferred.append({'behavior': behavior_name, 'key': _pf_key,
                                              'raw': _raw_proba,
                                              'clf_data': clf_data, 'clf_base': clf_base,
                                              'fps': fps,
                                              'smooth': _smooth, 'results_folder': results_folder,
                                              'bin_size': _bin_s,
                                              'summary_idx': len(summary_results) - 1})
                ...
                # ── Priority pass: every behavior scored in this run, highest
                #    priority first, from its raw probabilities against the
                #    final calls of the behaviors above it. Normally a no-op
                #    (the loop already ran in priority order); it re-derives
                #    anything scored before a behavior that overrides it.
                for _d in sorted(_deferred, key=lambda d: _prio_idx.get(d['behavior'], 1e9)):
                    _raw = _d['raw']
                    _mask = np.zeros(len(_raw), dtype=bool)
                    for _src in _excl_rules.get(_d['behavior'], []):
                        _m = _pf_cols.get(f"{_src}_pred") if _src in _in_batch else None
                        if _m is None:
                            continue
                        _n = min(len(_m), len(_raw))
                        _mask[:_n] |= np.asarray(_m[:_n]) == 1
                    _yp, _ypf = smooth_with_exclusions(_raw, _d['clf_data'], _d['smooth'], _mask)
                    _cur = _pf_cols.get(f"{_d['key']}_pred")
                    if _cur is not None and np.array_equal(np.asarray(_cur), np.asarray(_ypf)):
                        continue
                    _before = summary_results[_d['summary_idx']]['n_bouts']
                    _out = _write_clf_outputs(video_base, _d['clf_base'], _d['behavior'],
                                              _yp, _ypf, _d['fps'], _d['results_folder'],
                                              bin_size=_d.get('bin_size', 60.0))
                    _pf_cols[f"{_d['key']}_prob"] = _yp
                    _pf_cols[f"{_d['key']}_pred"] = np.asarray(_ypf)
                    summary_results[_d['summary_idx']].update(
                        n_bouts=_out['n_bouts'], percent_behavior=_out['percent_behavior'],
                        mean_bout_duration=_out['mean_bout_duration'])
                    self._batch_log(f"  Priority: {_d['behavior']} re-derived, zeroed on "
                                    f"{int(_mask.sum())} frames where a higher-priority behavior "
                                    f"fired ({_before} -> {_out['n_bouts']} bouts)\n")
                ...
                if ran and not _oc:
                    ...
                    messagebox.showinfo("Batch + analysis complete",
                        m + "\n\nGraphs are ready in the 📈 Single-Classifier Analysis tab."
                            "\n🏷 Review scoring (Run Classifiers / Tools) plays the calls on video.")
                elif not _oc:
            ...
            try:
                if self.root.winfo_exists():
                    ...
                    self.root.after(0, lambda: self._set_batch_list_buttons('normal'))
            except tk.TclError:
                pass
