"""
Post-DLC: extract features once per video and predict 5 behaviors
per the SNLT for_claude.json template — Facial_grooming, Left_licking,
rearing, still, walking — for every video in the THC_Withdrawal folder
that has a DLC h5 next to it.

Per-classifier post-processing (best_thresh, min_bout, min_after_bout,
max_gap) is applied from the JSON so the per-frame outputs match what
the SNLT transitions analysis expects.

Output layout, mirroring SNLT analysis structure:
  <PROJECT>/results/<session>_predictions.csv     ← 5 binary columns + proba
  <PROJECT>/features/<session>_features_<hash>.pkl ← cached for re-runs

Run with:
  PYTHONIOENCODING=utf-8 py -X utf8 scripts/research/thc_withdrawal_predict_all.py
"""
...
VIDEO_DIR = PROJECT_ROOT / 'Videos'   # PawCapture lays it out as 'Videos' (capital V)
JSON_PATH = Path(r'E:\RSVIDS\Blackbox\2603_SNLT_JG\Baseline\transitions\for_claude.json')
SHUFFLE = 9

# Use the local copies of the classifiers so the GUI sees them under
# this project's classifiers/ folder.
...
def apply_postprocess(y_pred, min_bout=1, min_after_bout=0, max_gap=0):
    """Mirror the transitions-tab post-processing applied per behavior."""
    import numpy as np
    y = y_pred.astype(int).copy()
    # Bridge gaps shorter than max_gap (fill 0-runs of length <= max_gap
    # between two 1-runs)
    if max_gap > 0:
        bouts = find_bouts(y)
    ...
    if min_bout > 1:
        for s, e in find_bouts(y):
            if e - s + 1 < min_bout:
                y[s: e + 1] = 0
    # Drop bouts whose preceding gap is shorter than min_after_bout
    if min_after_bout > 0:
        bouts = find_bouts(y)
...
def main() -> int:
    ...
    # Discover videos with DLC h5 next to them
    pairs = []
    ...
    # Output dirs (mirror SNLT layout)
    results_dir = PROJECT_ROOT / 'results'
    features_dir = PROJECT_ROOT / 'features'
    results_dir.mkdir(parents=True, exist_ok=True)
    features_dir.mkdir(parents=True, exist_ok=True)

    # Pre-load classifiers and compute union of bp_pixbrt. Prefer the
    # local project copies (classifiers/) over the absolute paths in
    # the JSON so this run is self-contained.
    loaded = []
    union_bp_pixbrt = []
    ...
    # Compute the canonical feature-cache hash. We extract WITH optical flow
    # so a single feature set serves both the no-flow classifiers (which just
    # ignore the flow columns via predict_with_xgboost's feature_names_in_
    # selection) AND the Scratching classifier (which uses flow). One pass
    # per video, no duplicate extraction.
    try:
    ...
    for i, (v, h5) in enumerate(pairs):
        ...
        # Canonical PixelPaws cache filename: <session>_features_<hash>.pkl
        # so the GUI's training/eval flow finds and reuses it.
        ...
        # Run each classifier and collect per-frame columns
    ...
    summary_csv = PROJECT_ROOT / 'thc_withdrawal_predict_summary.csv'
    summary_df.to_csv(summary_csv, index=False)
    step(f'\nSummary: {summary_csv}')
    discord_edit(msg_id, progress_text(total_sessions, total_sessions, t0_all))
    step('Done. Open the GUI Transitions tab and load '
         'for_claude.json (edit session_selection to THC sessions) for '
         'the full transitions analysis.')
    return 0
