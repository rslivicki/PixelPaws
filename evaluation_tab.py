...
def _apply_bout_filtering(y_pred, min_bout, min_after_bout, max_gap, polish_repeat=1):
    ...
    ``polish_repeat`` (default 1 = unchanged): run the whole
    ...
    (There is no separate ``min_after_gap`` knob: ``min_after_bout`` already
    provides a post-bridge refractory.)
...
def _labeled_mask(y_true):
    """True where a frame carries a real label (0/1).

    Unlabeled frames are -1 (or NaN) in PixelPaws label CSVs. They are not a
    class: they must never reach f1_score / confusion_matrix / bout counting.
    """
    yt = np.asarray(y_true, dtype=float)
    return np.isfinite(yt) & (yt >= 0)


def _mask_unlabeled(y_true, *arrays):
    """Drop unlabeled frames before a frame-level metric.

    Returns ``(y_true_int, *arrays)`` restricted to labeled frames. Every array is
    first truncated to the shortest length so they stay frame-aligned.
    """
    n = min([len(y_true)] + [len(a) for a in arrays])
    yt = np.asarray(y_true, dtype=float)[:n]
    obs = _labeled_mask(yt)
    return (yt[obs].astype(int),) + tuple(np.asarray(a)[:n][obs] for a in arrays)


def _zero_unlabeled(y_true, y_pred):
    """Set unlabeled frames to 0 in BOTH arrays (for bout metrics).

    Dropping them would splice their neighbours together and merge separate
    bouts; zeroing keeps the timeline and puts no bout on either side.
    """
    n = min(len(y_true), len(y_pred))
    yt = np.asarray(y_true, dtype=float)[:n]
    obs = _labeled_mask(yt)
    return (np.where(obs, np.nan_to_num(yt), 0).astype(int),
            np.where(obs, np.asarray(y_pred)[:n], 0).astype(int))


def _slices_from_lengths(lengths):
    """Half-open ``[(start, stop), ...]`` slices of sessions concatenated in order."""
    slices, start = [], 0
    for n in lengths:
        slices.append((start, start + int(n)))
        start += int(n)
    return slices


def _bout_filter_by_session(y_pred, slices, min_bout, min_after_bout, max_gap):
    """``_apply_bout_filtering`` inside each session slice, so max_gap never bridges
    bouts across a session boundary. ``slices=None`` filters the whole array."""
    if not slices:
        return _apply_bout_filtering(y_pred, min_bout, min_after_bout, max_gap)
    out = np.asarray(y_pred).copy()
    for s, e in slices:
        out[s:e] = _apply_bout_filtering(out[s:e], min_bout, min_after_bout, max_gap)
    return out


def _pool_bout_counts(bout_dicts):
    """Pool per-session ``_compute_bout_metrics`` dicts by summing their counts, so
    no bout is ever merged or matched across a session boundary."""
    n_true = sum(d['n_true_bouts'] for d in bout_dicts)
    n_pred = sum(d['n_pred_bouts'] for d in bout_dicts)
    matched_pred = sum(d['matched_pred'] for d in bout_dicts)
    matched_true = sum(d['matched_true'] for d in bout_dicts)
...
def count_bouts(y_pred: np.ndarray, fps: float) -> dict:
    ...
    for i, val in enumerate(y_pred):
        if val == 1 and not in_bout:
            bout_start = i
            in_bout = True
        elif val != 1 and in_bout:   # 0, -1 (unlabeled) or NaN all end a bout
            dur = i - bout_start
...
# ---------------------------------------------------------------------------
# Shared session discovery - used by training and evaluation
# ---------------------------------------------------------------------------
def find_session_triplets(
    ...
    ``require_labels=False`` (prediction).
    ...
    for dlc_path in sorted(dlc_files):
        ...
        dlc_candidates = rank_h5(glob.glob(os.path.join(dlc_dir, f'{base}DLC*.h5')))
...
class EvaluationTab(ttk.Frame):
    ...
    shared : dict, optional
        Set when this is the "Full evaluation" half of the merged Evaluate
        tab. The tab's shared controls then drive it and its own duplicates
        are hidden: 'classifier' (StringVar, full .pkl path), 'smoothing'
        (StringVar), 'read_overrides' (main-thread callable -> dict of
        best_thresh/min_bout/min_after_bout/max_gap, None, or False when
        the entries are invalid) and 'set_overrides' (callable(thr, mb, ma,
        mg), used by Optimize Parameters). No own scroll area either: the
        merged page scrolls.
    """

    def __init__(self, parent, app, shared=None):
        super().__init__(parent)
        self.app  = app
        self.root = app.root
        self._shared = shared
        self._shared_overrides = None   # snapshot taken on Run (main thread)
        ...
        self.eval_smoothing_mode    = (shared['smoothing'] if shared
                                       else tk.StringVar(value='bout_filters'))
        ...
        self._build_ui()
        if shared:
            shared['classifier'].trace_add(
                'write', lambda *_: self._sync_shared_classifier())
            self._sync_shared_classifier()

    def _sync_shared_classifier(self):
        """Follow the merged tab's classifier picker (it holds the full path
        once a classifier is chosen or browsed to)."""
        p = self._shared['classifier'].get().strip()
        if p and os.path.isfile(p) and p != self.eval_classifier_path.get():
    ...
    def _build_ui(self):
        ...
        # Scrollable canvas (none when embedded: the merged page scrolls)
        if self._shared:
            canvas = scrollbar = None
            sf = self
        else:
        ...
        if self._shared:
            # The merged tab's Classifier box, Smoothing choice and
            # threshold / bout-filter overrides drive this half, so its own
            # copies stay built (other code writes to them) but hidden.
            sel.pack_forget()
            info_frame.pack_forget()
            for w in opt.grid_slaves():
                if int(w.grid_info().get('row', 0)) >= 4 or w is smooth_frame:
                    w.grid_remove()
            ttk.Label(opt, text='Classifier, smoothing and threshold / bout-filter '
                                'overrides come from the top of this tab.',
                      foreground='gray').grid(row=10, column=0, columnspan=3,
                                              sticky='w', pady=(6, 0))
            self.eval_results_text.configure(height=12)
            return
    ...
    def run_evaluation(self):
        ...
        if not self.eval_test_folder.get():
            ...
            return
        if self._shared:
            ov = self._shared['read_overrides']()
            if ov is False:          # invalid entries; the reader said why
                return
            self._shared_overrides = ov
    ...
    def _evaluation_thread(self):
        ...
        try:
            ...
            if self._shared:
                _ov = self._shared_overrides or {}
                if _ov:
                    best_thresh = _ov.get('best_thresh', best_thresh)
                    min_bout    = _ov.get('min_bout', min_bout)
                    min_after   = _ov.get('min_after_bout', min_after)
                    max_gap     = _ov.get('max_gap', max_gap)
                    self._log(f'  [Custom overrides] threshold={best_thresh:.3f}, '
                              f'min_bout={min_bout}, min_after={min_after}, max_gap={max_gap}')
            elif self.eval_use_custom_params.get():
            ...
            # ── Per-session loop ─────────────────────────────────────────
            all_y_true        = []   # labeled frames only
            all_y_pred        = []
            all_bout_m        = []   # per-session bout counts (pooled below)
            per_video_results = []
            ...
            for idx, session in enumerate(sessions, 1):
                ...
                # Unlabeled frames (-1 / NaN) become NaN and are excluded
                # from every metric below.
                y_true = pd.to_numeric(labels_df[label_col],
                                       errors='coerce').to_numpy(dtype=float)
                y_true[~_labeled_mask(y_true)] = np.nan
                if not _labeled_mask(y_true).any():
                    self._log('  ✗ No labeled frames for this behavior (all -1/NaN)  - skipping.')
                    continue
                ...
                # apply_smoothing reads threshold and bout filters from the
                # dict it gets, so the custom overrides must live on a copy
                # (as in Run Classifiers); unticking "Apply bout filtering"
                # scores the thresholded frames with no bout filter.
                _eff = dict(clf_data)
                _eff.update({'best_thresh': best_thresh, 'min_bout': min_bout,
                             'min_after_bout': min_after, 'max_gap': max_gap})
                try:
                    from prediction_pipeline import apply_smoothing as _smooth_fn
                    _smode = self.eval_smoothing_mode.get()
                    if not apply_bout_filter and _smode == 'bout_filters':
                        _smode = 'none'
                    y_pred = _smooth_fn(y_proba, _eff, _smode)
                ...
                # ── Per-video metrics (labeled frames only) ──────────────
                yt_obs, yp_obs = _mask_unlabeled(y_true, y_pred)
                if len(yt_obs) == 0:
                    self._log('  ✗ No labeled frames in the aligned range  - skipping.')
                    continue
                if len(yt_obs) < n:
                    self._log(f'  {n - len(yt_obs)} unlabeled frame(s) (-1/NaN) '
                              f'excluded from metrics.')
                f1, prec, rec, acc = self._compute_metrics(yt_obs, yp_obs)
                bout_m = self._compute_bout_metrics(*_zero_unlabeled(y_true, y_pred))
                self._log(
                ...
                all_y_true.extend(yt_obs.tolist())
                all_y_pred.extend(yp_obs.tolist())
                all_bout_m.append(bout_m)

                per_video_results.append({
                    'video':      video_name,
                    'n_frames':   len(yt_obs),
                    'n_true':     int(np.sum(yt_obs)),
                    'n_pred':     int(np.sum(yp_obs)),
                    'f1':         f1,
                    'precision':  prec,
                    'recall':     rec,
                    'accuracy':   acc,
                ...
                if self.eval_save_predictions.get():
                    ...
                    pd.DataFrame({
                        'Frame':       np.arange(n),
                        # -1 = unlabeled (the label-CSV convention)
                        'y_true':      np.where(_labeled_mask(y_true),
                                                np.nan_to_num(y_true), -1).astype(int),
                        'y_pred':      y_pred,
            ...
            self._log(f'Labeled frames:      {len(all_y_true)}')
            self._log(
            ...
            # Aggregate bout-level metrics: per-session counts summed, so bouts
            # never merge across a session boundary or an unlabeled stretch.
            ov_bout = _pool_bout_counts(all_bout_m)
            self._log(f'\n--- Bout-Level Metrics ---')
    ...
    @staticmethod
    def _grid_search_params(y_proba, y_true, metric='f1', session_slices=None):
        """Grid search over threshold/bout-filter params on pre-computed probabilities.

        ``y_true`` may hold unlabeled frames (-1 / NaN): bout filtering still runs
        on the full timeline, then those frames are dropped before scoring.
        ``session_slices`` (``[(start, stop), ...]`` over the concatenated arrays)
        keeps bout filtering inside each session, so max_gap never bridges bouts
        ...
        obs = _labeled_mask(y_true)
        y_true = np.asarray(y_true, dtype=float)[obs].astype(int)
            ...
            for mb in min_bouts:
                for ma in min_after_bouts:
                    for mg in max_gaps:
                        ...
                        y_pred = _bout_filter_by_session(
                            y_pred, session_slices, mb, ma, mg)[obs]
    ...
    def _cv_optimization_thread(self, sessions, log_fn, done_fn):
        ...
        try:
            ...
            for idx, session in enumerate(sessions, 1):
                ...
                # Unlabeled frames (-1 / NaN) stay NaN; they are dropped before
                # every metric but kept so bout filtering sees the real timeline.
                y_true = pd.to_numeric(labels_df[label_col],
                                       errors='coerce').to_numpy(dtype=float)
                y_true[~_labeled_mask(y_true)] = np.nan
                ...
                if not _labeled_mask(y_true[:n]).any():
                    log_fn('  Skipping - no labeled frames (all -1/NaN)')
                    continue
                video_data.append({
                    'y_proba': y_proba[:n],
                    'y_true': y_true[:n],
            ...
            # Step 2: LOO grid search. Sessions are concatenated with their
            # boundaries kept (session slices), so bout filtering never bridges two.
            lengths = [len(v['y_true']) for v in video_data]
            fold_results = []
            for k in range(N):
                train_ids = [i for i in range(N) if i != k]
                train_proba = np.concatenate(
                    [video_data[i]['y_proba'] for i in train_ids])
                train_labels = np.concatenate(
                    [video_data[i]['y_true'] for i in train_ids])

                best = self._grid_search_params(
                    train_proba, train_labels,
                    session_slices=_slices_from_lengths([lengths[i] for i in train_ids]))
                best['train_f1'] = best['f1']

                # Score the held-out session with the params picked on the others
                _ho = video_data[k]
                _ho_pred = (_ho['y_proba'] >= best['threshold']).astype(int)
                _ho_pred = _apply_bout_filtering(
                    _ho_pred, best['min_bout'], best['min_after_bout'], best['max_gap'])
                (best['f1'], best['precision'], best['recall'],
                 best['accuracy']) = self._compute_metrics(
                    *_mask_unlabeled(_ho['y_true'], _ho_pred))
                _ho_bout = self._compute_bout_metrics(
                    *_zero_unlabeled(_ho['y_true'], _ho_pred))
                best['bout_f1'] = _ho_bout['bout_f1']

                fold_results.append(best)

                log_fn(f'\n  Fold {k+1}/{N} (held out: {_ho["name"][:30]})')
                log_fn(f'    Best on training sessions: thresh={best["threshold"]:.3f}, '
                       f'min_bout={best["min_bout"]}, min_after={best["min_after_bout"]}, '
                       f'max_gap={best["max_gap"]}  (train F1={best["train_f1"]:.3f})')
                log_fn(f'    Held-out: F1={best["f1"]:.3f}  '
            ...
            # (bout filtering + bout counting per session; unlabeled frames excluded)
            all_slices = _slices_from_lengths(lengths)
            all_y_true = np.concatenate([v['y_true'] for v in video_data])
            all_y_proba = np.concatenate([v['y_proba'] for v in video_data])
            all_y_pred = _bout_filter_by_session(
                (all_y_proba >= avg_thresh).astype(int), all_slices,
                avg_mb, avg_ma, avg_mg)

            f1, prec, rec, acc = self._compute_metrics(
                *_mask_unlabeled(all_y_true, all_y_pred))

            lovo_bout = _pool_bout_counts([
                self._compute_bout_metrics(
                    *_zero_unlabeled(all_y_true[s:e], all_y_pred[s:e]))
                for s, e in all_slices])
            ...
            def_y_pred = _bout_filter_by_session(
                (all_y_proba >= default_thresh).astype(int), all_slices,
                default_mb, default_ma, default_mg)
            def_f1, def_prec, def_rec, def_acc = self._compute_metrics(
                *_mask_unlabeled(all_y_true, def_y_pred))
            ...
            log_fn('=' * 60)
            log_fn('  (F1 and BoutF1 are on the held-out session; TrainF1 is the '
                   'grid-search score on the other sessions)')
            ...
            for k, r in enumerate(fold_results):
                log_fn(f'  {k+1:>5d}  {r["threshold"]:>10.3f}  {r["min_bout"]:>8d}  '
                       ...
                       f'{r.get("bout_f1", 0):>7.3f}  {r.get("train_f1", 0):>7.3f}')

            log_fn(f'\n✓ Optimization complete!')

            # Populate custom parameter spinboxes (or, in the merged tab,
            # the shared threshold / bout-filter override fields)
            if self._shared:
                self._safe_after(lambda: self._shared['set_overrides'](
                    avg_thresh, avg_mb, avg_ma, avg_mg))
            else:
