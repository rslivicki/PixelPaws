#!/usr/bin/env python3
"""
PawCapture — multi-camera recording controller for PixelPaws.

OpenCV owns the single capture handle per camera (so exposure/gain props
always work); FFmpeg does the recording (hardware encode where available)
and writes the mm/pixel calibration + session manifest that PixelPaws reads.

Run from source:  python pawcapture_app.py
Build a portable folder / installer:  build.bat  (see README.md)
"""
import os, sys, json, subprocess, threading, time, shutil, re
...
class CameraThread(QThread):
    ...
    # Non-fatal: the capture loop keeps running (e.g. encode pipeline failed
    # and we reverted to preview-only). The panel must stay CONNECTED.
    camera_warning = pyqtSignal(str)
    ...
    # Emitted when FFmpeg died mid-recording and the thread reopened the
    # camera, continuing into a new part file. Args: (new_path, part_number).
    recording_resumed = pyqtSignal(str, int)

    # Auto-resume guards. A recording FFmpeg that crashes after running for at
    # least RESUME_MIN_UPTIME_S is resumed immediately (a one-off crash, e.g.
    # the 2026-09-15 0xc0000005 on CAM 1). One that dies sooner counts as a
    # quick failure; after RESUME_MAX_QUICK_FAILS in a row we stop retrying and
    # surface the error, since that pattern means the camera is gone or FFmpeg
    # can't start at all, and looping would just spin.
    RESUME_MIN_UPTIME_S   = 15.0
    RESUME_MAX_QUICK_FAILS = 3
    RESUME_SETTLE_S       = 1.0
    ...
    @staticmethod
    def _next_part_path(base_path, start=2):
        """Path for the next continuation of a crashed recording:
        `<stem>_pt2.mp4`, `_pt3`, ... beside the original, skipping any part
        that already exists so a resume can never record over a file.
        Returns (path, part_number)."""
        base = Path(base_path)
        n = start
        while True:
            cand = base.with_name(f"{base.stem}_pt{n}{base.suffix}")
            if not cand.exists():
                return cand, n
            n += 1

    def _resume_recording_after_crash(self):
        """Point _record_args at the next part file and flag a respawn.
        Returns (path, part) or None if there is no recording to resume, the
        user stopped it, or a respawn is already pending."""
        with self._lock:
            ra = self._record_args
            if not ra or self._respawn_pending:
                return None
            base = ra.get("base_path") or ra["path"]
            path, part = self._next_part_path(base, start=ra.get("part", 1) + 1)
            meta = dict(ra.get("metadata") or {})
            meta["pawcapture_part"] = str(part)
            meta["pawcapture_part_of"] = Path(base).name
            self._record_args = dict(ra, path=path, base_path=base,
                                     part=part, metadata=meta)
            return path, part
    ...
    def _run_inner(self):
        ...
        if ff_proc:
            ...
            # (record toggle).  (c) re-enters the loop; (b) re-enters only when
            # a recording was in progress and can be resumed into a new part.
            _ff_started      = time.monotonic()
            _quick_fails     = 0
            while self.running:
                ...
                while self.running:
                    ...
                    if len(raw) == frame_bytes:
                        ...
                        # EOF on stdout usually lands a moment before the OS
                        # reaps the process, so poll() can still say "running".
                        # Unless we asked it to stop, give it a moment to exit
                        # - otherwise a crash is misread as a requested
                        # respawn, which re-launches the recording onto the
                        # SAME path and stalls on FFmpeg's overwrite prompt
                        # (the ~35 s gap seen after the 2026-09-15 CAM 1 crash).
                        if ff_proc.poll() is None and not self._respawn_pending:
                            try: ff_proc.wait(timeout=3.0)
                            except Exception: pass
                ...
                if _ffmpeg_died and not self._respawn_pending:
                    # FFmpeg died on its own. If it was recording, reopen the
                    # camera and continue into <name>_ptN.mp4 rather than
                    # silently ending that camera's recording. The fragmented
                    # mp4 already on disk stays playable up to the crash.
                    uptime = time.monotonic() - _ff_started
                    _quick_fails = 0 if uptime >= self.RESUME_MIN_UPTIME_S else _quick_fails + 1
                    if _quick_fails < self.RESUME_MAX_QUICK_FAILS:
                        resumed = self._resume_recording_after_crash()
                    if resumed is None:
                        # Not recording, or crashing too fast to keep retrying
                        # - surface and exit Phase 1.
                        try:
                            tail = ff_proc.stderr.read(500).decode(errors="replace").strip()
                        except Exception:
                            tail = ""
                        msg = "FFmpeg capture stopped."
                        if self._record_args and _quick_fails >= self.RESUME_MAX_QUICK_FAILS:
                            msg += (f"\nRecording could not be resumed: FFmpeg failed "
                                    f"{_quick_fails} times in a row within "
                                    f"{self.RESUME_MIN_UPTIME_S:.0f}s of starting.")
                        self.camera_error.emit(f"{msg}\n{tail}" if tail else msg)
                        break
                    # Give the device a moment to settle, and reopen the COM
                    # property interfaces that were closed when FFmpeg died.
                    time.sleep(self.RESUME_SETTLE_S)
                    try: self._dshow_ctrl.open()
                    except Exception: pass
                    if not self.running:
                        break
                ...
                if not ff_proc and self._record_args:
                    # Recoverable: the thread keeps running in preview-only
                    # mode, so this must NOT go out as camera_error (which
                    # resets the panel to CONNECT while we are still alive;
                    # the next CONNECT then drops a running QThread).
                    self.camera_warning.emit(
                ...
                self._ffmpeg_proc = ff_proc
                _ff_started = time.monotonic()
                if resumed is not None and self._record_args:
                    self.recording_resumed.emit(str(resumed[0]), resumed[1])
...
class Recorder:
    ...
    def __init__(self, label):
        self.label       = label
        self.path        = None
        # Every file this recording produced, in order. Normally just [path];
        # grows with <stem>_pt2.mp4, _pt3 ... if FFmpeg crashed and the
        # CameraThread resumed recording into a continuation file.
        self.parts       = []
        self.active      = False
        self.codec_used  = ""
    ...
    def write(self, frame):
        ...
        if self._inline:
            return
        # Snapshot the process handle: stop() runs on the GUI thread and sets
        # self.proc = None, which could land between the guard below and the
        # poll()/write() calls and raise AttributeError on the capture thread.
        proc = self.proc
        if not (self.active and proc and proc.stdin): return
        ...
        try:
            ...
            # ValueError: write on a stdin that stop() closed under us.
            self.active = False
...
class CameraPanel(QWidget):
    ...
    def _connect(self):
        ...
        self.thread.camera_warning.connect(self._on_camera_warning)
    ...
    def _on_recording_resumed(self, path, part):
        """FFmpeg crashed mid-recording and the thread reopened the camera,
        continuing into a new part file. Track it for the session manifest
        and make the break visible - it's otherwise silent."""
        if not self.recorder.active:
            return
        self.recorder.parts.append(Path(path))
        stamp = datetime.now().strftime("%H:%M:%S")
        self.rec_lbl.setText(f"⚠ REC  pt{part}")
        self.rec_lbl.setToolTip(
            f"FFmpeg crashed at {stamp}; recording resumed into\n{path}\n"
            "Earlier part(s) are intact up to the crash.")
        win = self.window()
        if hasattr(win, "sb"):
            win.sb.showMessage(
                f"⚠ {self.label}: FFmpeg crashed at {stamp} - recording "
                f"resumed as {Path(path).name}")

    def _on_error(self, msg):
        # A stale error queued from a thread we already replaced must not
        # tear down the current one.
        src = self.sender()
        if src is not None and self.thread is not None and src is not self.thread:
            return
        # Fatal: the capture loop is exiting (or already gone). Stop and join
        # the thread *before* flipping the panel to CONNECT, otherwise the next
        # CONNECT builds a new CameraThread and drops the reference to this
        # one while it may still be running ("QThread: Destroyed while thread
        # is still running" aborts the whole app) and opens the same dshow
        # device twice.
    ...
    def _on_camera_warning(self, msg):
        """Recoverable capture-side problem (e.g. the encode pipeline failed
        and the thread fell back to preview-only). The CameraThread is still
        running, so the panel stays CONNECTED; only the recording UI resets."""
        src = self.sender()
        if src is not None and self.thread is not None and src is not self.thread:
            return
        if self.recorder.active or self._rec_start:
            # The thread already cleared _record_args and respawned in
            # preview-only mode; drop the inline flag so stop_rec() only
            # resets the timer/labels instead of requesting another respawn.
            self.recorder._inline = False
            self.stop_rec()
        QMessageBox.warning(self, "Recording Stopped",
            f"{self.label}: {msg}\n\nThe camera preview is still live.")
    ...
    def start_rec(self):
        ...
        if ok:
            ...
            self.rec_lbl.setText("● REC"); self.rec_lbl.setToolTip("")
...
<ul>
  ...
  <li>If FFmpeg crashes mid-recording, that camera reopens and keeps recording into <code>&lt;name&gt;_pt2.mp4</code> (then <code>_pt3</code> …) beside the first file, which stays playable up to the crash. The panel shows <b>⚠ REC pt2</b> and the status bar says when. Three crashes within 15 s of starting stop the retries and show an error.</li>
</ul>
...
<ul>
  ...
  <li>Per-camera files, calibration, resolution/fps/crop, and encoder used (<code>file_parts</code> lists every part when a camera resumed after a crash)</li>
...
<p>Drop <code>pawcapture_meta.py</code> (next to <code>pawcapture_app.py</code> in the source tree) into PixelPaws — no install needed. Provides:</p>
...
<ul>
  ...
  <li><b>If a camera shows a tiled / mosaic pattern:</b> unplug and re-plug its USB to
      reset the ISP. Don't run two PawCapture instances against the same camera.</li>
</ul>
...
class MainWindow(QMainWindow):
    ...
    def _stop_recording_session(self):
        ...
        for p in self._session_panels:
            ...
            if path and str(path) != "None":
                ...
                # A crash mid-recording splits one camera's recording across
                # <stem>.mp4 + <stem>_pt2.mp4 ...  `file` stays the first part
                # for compatibility; `file_parts` lists all of them in order.
                parts = [str(x) for x in getattr(p.recorder, "parts", [])]
                if len(parts) > 1:
                    cam_data["file_parts"] = parts
                    cam_data["resumed_after_crash"] = len(parts) - 1
    ...
    def _start_recording_session(self):
        ...
        collisions = []
        # Also refuse two live panels resolving to the same target (same File
        # name + folder within the same second): two FFmpegs would mux one
        # file and the second would stall on the overwrite prompt.
        duplicates = []
        seen = {}
        for p in live:
            ...
            key = os.path.normcase(os.path.abspath(str(t)))
            if key in seen:
                duplicates.append(f"{seen[key]} and {p.label}:  {t.name}")
            else:
                seen[key] = p.label
        if collisions:
            QMessageBox.warning(self, "Filename Already Taken",
        ...
        if duplicates:
            QMessageBox.warning(self, "Filename Already Taken",
                "These cameras would record to the same file:\n\n"
                + "\n".join(duplicates)
                + "\n\nGive each camera its own File name or folder and try again.")
            return
        errors = []; started = []
        for p in live:
            ok, err = p.start_rec()
    ...
    def save_profile(self):
        ...
        # Atomic write: a crash mid-write must not leave a truncated profile
        # that then aborts the app every time it is selected.
        ...
        try:
            ...
            QMessageBox.critical(self, "Save Profile",
                f"Could not save profile '{target_name}':\n\n{e}")
            return
    ...
    def load_profile(self):
        ...
        # A truncated or hand-edited profile must not abort the app (this is
        # a Qt slot: an uncaught exception here kills the process on every
        # selection of that profile).
        try:
            data = json.loads(path.read_text())
            cam_list = data.get("cameras", [])
            if not isinstance(cam_list, list):
                raise ValueError("'cameras' is not a list")
        except Exception as e:
            QMessageBox.critical(self, "Load Profile",
                f"Profile '{name}' could not be read:\n\n{e}\n\n{path}\n\n"
                "Fix or delete the file, or save a new profile over it.")
            return
    ...
    def closeEvent(self, e):
        if self.is_recording:
            # Go through the session stop so the session_*.json manifest
            # (sync marks, OFRS pairing, calibration) is written; a bare
            # per-panel stop_rec() finalizes the mp4s but loses all of that.
            try:
                self._stop_recording_session()
            except Exception as ex:
                print(f"[PawCapture] session stop on close failed: {ex}")
                for p in self.panels:
                    try: p.stop_rec()
                    except Exception: pass
                self.is_recording = False
        # (_disconnect() below stops any per-panel RECORD THIS CAM recording.)
