...
# PyInstaller spec for PawCapture (pawcapture_app.py).
#
...
# (or just run build.bat from the pawcapture directory)
...
if not FFMPEG_SRC.exists():
    raise SystemExit(
        ...
        "Drop the official Windows ffmpeg build into pawcapture\\ffmpeg\\ (build.bat downloads it)."
    )
...
a = Analysis(
    ...
    # ffmpeg is a GPL build (gyan.dev release-essentials); its license text
    # ships next to the binary. build.bat copies it out of the download.
    datas=[(str(ROOT / "ffmpeg" / "FFMPEG_LICENSE.txt"), '.')]
          if (ROOT / "ffmpeg" / "FFMPEG_LICENSE.txt").exists() else [],
    hiddenimports=[],
    hookspath=[],
    runtime_hooks=[],
    excludes=[
