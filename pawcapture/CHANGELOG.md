...
## 2026-09-18: 0.8.0: recording resumes after an FFmpeg crash

Merged from the camsync rig's 2026-09-15 change (DESKTOP-4331DDR). On 2026-09-15 CAM 1's
ffmpeg.exe died mid-recording with an access violation (0xc0000005) about 21 min into a
post-drug run, and on 2026-09-09 the same camera did the same at 18:20 of a baseline; the
other cameras kept going and the short clip was only noticed later. The crash itself is
still unexplained (CAM 1 is the only rot-270 camera; all cameras use h264_qsv).

- **Why recording never came back:** on stdout EOF the capture loop called `poll()` at once,
  before the OS had reaped the crashed process, so the death read as a requested respawn.
  The respawn relaunched recording onto the SAME path, FFmpeg (no `-y`) waited at its
  overwrite prompt, and the first-byte probe timed out three times before falling back to
  preview-only. EOF now waits up to 3 s for the process to exit before deciding.
- **Auto-resume:** an unrequested FFmpeg death while recording points the thread at
  `<stem>_pt2.mp4` (`_pt3`, ... skipping files that exist) beside the original, stamps
  `pawcapture_part` / `pawcapture_part_of` metadata, reopens the COM property control and
  respawns. New signal `recording_resumed(path, part)`; the panel shows "⚠ REC pt2" and the
  status bar says when.
- **Crash-loop guard:** a death within 15 s of starting is a quick failure; three in a row
  stop the retries and show an error that says so.
- **Manifest:** `Recorder.parts` tracks every file; `file` stays the first part (PixelPaws
  compatibility) and `file_parts` + `resumed_after_crash` are added when a camera resumed.
- In-app HELP and README: the Recording section describes the resume; the README now
  carries the full walkthrough from HELP.
- Tests: `tests/pawcapture/test_crash_autoresume.py` (fake FFmpeg that exits 0xC0000005,
  no cameras): resume into _pt2, numbering skips existing parts, crash loop gives up,
  preview-only crash keeps the old error, stop racing a crash does not resume.
  **Not yet verified against real cameras.**
- Known, not fixed here: post-drug phases often end without a `session_*.json` manifest
  (the post-drug stop path seems to skip session finalize; intermittent), and the Discord
  error webhook has returned 403 since at least August.

---

## 2026-09-16: 0.7.0: PawCapture branding everywhere, one build script

Prep for the public PixelPaws release (PawCapture now lives in `pawcapture/` of the
PixelPaws repo and ships on the same Releases page).

- `camsync_precursor.py` → **`pawcapture_app.py`**; help text and docstring updated.
- **One `build.bat`** replaces `build.ps1` + `build_installer.bat` (which disagreed on the
  spec file and exe name): downloads ffmpeg if missing, PyInstaller onedir via
  `PawCapture.spec`, zips `dist\PawCapture-<ver>_win64.zip`, and builds the Inno Setup
  installer when ISCC is present. Version is read from `PAWCAPTURE_VERSION`.
- `installer.iss` packages the onedir folder as **PawCapture** (`PawCapture_Setup_v<ver>.exe`)
  instead of the single-file "CamSync Pro" exe; same AppId so existing installs upgrade in
  place, and the old `CamSyncPro.exe` is removed.
- `requirements.txt` gains `comtypes` (imported for DirectShow property pages; was only
  installed implicitly by the old installer script).
