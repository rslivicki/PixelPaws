...
Mouse licks its left hindpaw, typically a pain-/itch-related response.

## v1 status
- **CV-F1 (mean ± std)**: 0.906 ± 0.026
- **Inclusion gate**: 0.85 frame F1 — **PASSES** (0.906 ≥ 0.85)
- **Status**: KEEP — shipped as-is in v1 (carried over from `260506_RS_THC_Withdrawal` cohort).

## Training data
- Cohort: `260506_RS_THC_Withdrawal`
- Sessions: 3 (per-session list in the source pkl's
...
## Post-processing
- best_thresh: 0.5299999999999998
- min_bout: 3
- min_after_bout: 1
- max_gap: 5
...
- Performance panels (threshold curve, SHAP beeswarm, confusion matrix):
  ...
  `../performance_results/Left_licking/`.
...
## Source pkl
Original: `260506_RS_THC_Withdrawal` cohort. See `../classifiers/classifier_Left_licking.pkl`
