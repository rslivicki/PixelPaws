...
class Labeler:
    def __init__(self, root):
        ...
        self.prefill = {}
        self.i = 0
        # default hlpaw; a sidecar bps list may lack it (or be shorter than 6)
        self.active = BPS.index('hlpaw') if 'hlpaw' in BPS else 0
        ...
        self._build_ui()
        self._bind()
        # A bad folder (no CollectedData_<scorer>.h5, or a config bodypart the h5 lacks) used to
        # raise out of __init__ and kill the labeler before the window appeared. load_next_folder
        # skips such folders; anything else is reported and the window stays up so another
        # subject can be picked from the box.
        try:
            loaded = self.load_next_folder()
        except Exception as e:
            loaded = None
            messagebox.showerror("Load failed", f"Could not open the labeling folders:\n{e}")
        if loaded is False:
    ...
    def load_next_folder(self):
        """Open the first folder that still needs labeling. A folder that fails to load is
        skipped (one error box names each) so it can't block the rest. Returns True when a
        folder is open, False when none needs labeling, None when every candidate failed."""
        ...
        if failed:
            messagebox.showerror("Load failed", "Could not open (skipped):\n\n" + "\n\n".join(failed))
        if not opened:
    ...
    def load_folder(self, name):
        # Read everything into locals first and commit only once it all succeeded: a failed load
        # used to leave self.folder/self.dir pointing at the new folder with the old coords, so
        # the next save wrote one subject's labels into another's CollectedData.
        d_dir = os.path.join(LDIR, name)
        imgs = sorted(os.path.basename(p) for p in glob.glob(os.path.join(d_dir, "img*.png")))
        if not imgs:
            raise FileNotFoundError(f"no img*.png frames in {d_dir}")
        cd = pd.read_hdf(os.path.join(d_dir, f"CollectedData_{SCORER}.h5"))
        sc = cd.columns.get_level_values(0)[0]
        missing = [bp for bp in BPS if (sc, bp, 'x') not in cd.columns or (sc, bp, 'y') not in cd.columns]
        if missing:
            raise ValueError(f"CollectedData_{SCORER}.h5 has no columns for {', '.join(missing)} "
                           f"(the labeler config lists bodyparts this folder was not labeled with)")
        ...
        self.i = 0; self.sel = None; self.undo_stack = []; self._dirty = False
        self.load_frame()
        self._refresh_folder_menu()

    def load_frame(self):
        if not self.imgs:           # no folder opened (startup load failed); nothing to show
            self.i = 0; return
        self.i = max(0, min(self.i, len(self.imgs) - 1))
    ...
    def folder_done(self):
        ...
        # None = the remaining folders all failed to load (already reported); stay open
        if self.load_next_folder() is False:
            messagebox.showinfo("Done", "All folders labeled! The labels are ready to merge and retrain.")
            self.root.destroy()
