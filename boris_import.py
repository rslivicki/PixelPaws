...
the session's feature cache, using three-valued labels:
   ...
   -1   NOT reviewed / unobserved (excluded from training)
...
exported data correct "at the outset" so training does not ingest false negatives.

The core ``export_boris_labels`` is used by the GUI "Import BORIS Project" and
QC tools, and by the command line at the bottom of this file.
"""
import os
import sys
import json
...
def parse_boris(boris_path, with_types=False):
    """Load a `.boris` JSON project → (behaviors, observations dict).

    ``with_types=True`` returns ``(behaviors, observations, behavior_types)`` where
    ``behavior_types[code]`` is BORIS's type string ("Point event" / "State event").
    """
    with open(boris_path, encoding="utf-8", errors="replace") as f:
        proj = json.load(f)
    conf = (proj.get("behaviors_conf") or {}).values()
    behaviors = [v.get("code") for v in conf if v.get("code")]
    if with_types:
        types = {v.get("code"): str(v.get("type") or "") for v in conf if v.get("code")}
        return behaviors, proj.get("observations", {}), types
...
def _is_point(behavior, behavior_types):
    """True when BORIS declares ``behavior`` a POINT event (a single frame, no STOP)."""
    return "point" in str((behavior_types or {}).get(behavior, "")).lower()


def _state_bouts(events, behavior, fps):
    """Pair the START/STOP toggles of a STATE behavior per subject.

    BORIS toggles a state per (subject, behavior), so toggles are paired within each
    subject and never across subjects: two subjects' overlapping bouts no longer
    cancel each other. Returns ``(bouts, n_unclosed)``; ``bouts`` is a list of
    ``(start, stop)`` frames and ``n_unclosed`` counts subjects left with a dangling
    START (dropped).
    """
    by_subject = {}
    for ev in events:
        if ev[2] == behavior:
            by_subject.setdefault(ev[1], []).append(_event_frame(ev, fps))
    bouts = []
    n_unclosed = 0
    for frames in by_subject.values():
        frames.sort()
        for i in range(len(frames) // 2):
            bouts.append((frames[2 * i], frames[2 * i + 1]))
        n_unclosed += len(frames) % 2
    return bouts, n_unclosed


def build_label_frame(events, behaviors, N, fps, per_behavior_tails=True,
                      subjects=None, behavior_types=None):
    ...
    ``behavior_types`` (optional ``{code: BORIS type}``, see ``parse_boris``): POINT
    behaviors mark one frame per event. STATE behaviors (the default) pair START/STOP
    per subject, and bouts from different subjects are OR-ed together.
    """
    ...
    for b in behaviors:
        ...
        if observed_to < N:
            arr[observed_to:] = -1
        if _is_point(b, behavior_types):
            # POINT events are single frames, never a bout to the next point.
            for f in frames:
                if 0 <= f < N:
                    arr[f] = 1
        else:
            # Fill state bouts (half-open [start, stop)), paired per subject.
            for s, e in _state_bouts(events, b, fps)[0]:
                if e < s:
                    s, e = e, s
...
def export_boris_labels(boris_path, out_dir, feat_dir=None,
    ...
    """
    all_behaviors, observations, beh_types = parse_boris(boris_path, with_types=True)
    ...
    for obs_key, o in observations.items():
        ...
        # Warn on any state behavior with an unclosed START (odd number of toggles
        # for one subject). POINT events have no STOP, so they are never unclosed.
        _ev_for_warn = (events if subj_filter is None
                        else [ev for ev in events if ev[1] in subj_filter])
        warnings = []
        for b in use_behaviors:
            if _is_point(b, beh_types):
                continue
            n_open = _state_bouts(_ev_for_warn, b, fps)[1]
            if n_open:
                warnings.append(f"{b}: unclosed START dropped"
                                + (f" ({n_open} subjects)" if n_open > 1 else ""))

        df, summary = build_label_frame(events, use_behaviors, N, fps,
                                        per_behavior_tails, subjects=subj_filter,
                                        behavior_types=beh_types)
...
def _verify_against_boris(boris_path, out_dir, feat_dir, per_behavior_tails, log,
    ...
    for obs_key, o in observations.items():
        ...
        ref, _ = build_label_frame(events, use_behaviors, N, fps, per_behavior_tails,
                                   subjects=subj_filter, behavior_types=beh_types)
        got = pd.read_csv(csv)
        if len(got) != N:
