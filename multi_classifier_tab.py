...
class MultiClassifierTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        _T(ttk.Button(df_, text="🏷 Review scoring…",
           ...
           "Play a session with every classifier's calls drawn on the video "
           "(opens on the session selected above).").pack(anchor="w", padx=6,
                                                            pady=(2, 0))
    ...
    def _open_scoring_preview(self):
        """Review scoring (classifier calls on video) for the selected session."""
        from scoring_preview import open_scoring_preview
        proj = getattr(self.app, "current_project_folder", None)
        proj = proj.get() if proj is not None else ""
        open_scoring_preview(self.winfo_toplevel(), proj,
                             self._session_var.get() or None)
    ...
    def _group_of(self, session):
        ...
        for _, r in self._key_df.iterrows():
            if r["Subject"] in toks:
                return r["Treatment"]
        # Multi-token subjects (e.g. "cohortA_day3_S1"): match on
    ...
    def _states_for(self, session):
        ...
        `prio` is the full priority list (the union of behaviors across
        sessions), the same for every session; a behavior this session was
        not scored with simply never fires (zero-filled), so occupancy rows
        line up across sessions scored with different classifier sets."""
        d = self._sheet(session)
        prio = self._priority()
        state = np.zeros(len(d), np.int16)
        for idx in range(len(prio) - 1, -1, -1):
            col = f"{prio[idx]}_pred"
    ...
    def _draw_occupancy(self):
        ...
        if not sessions:
            ...
            ax.set_axis_off()
            return
        prio = self._priority()       # one label set for every session
        k = len(prio)
        occ = {}                                    # {session: fractions}
        for s in sessions:
            st, _p = self._states_for(s)
